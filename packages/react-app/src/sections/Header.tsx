...
function Header({ isDarkMode, darkModeToggle }) {
    ...
    const updateHeaderTag = (location) => {
        if (navigationSetup) {
            Object.entries(navigationSetup.navigation).forEach(
                ([key, value]) => {
        ...
        <Container direction="row" padding="0px 15px">
                ...
                {navigationSetup && showNavBar && active && !error && (
                    <NavMenuContainer align="flex-start">
                        <NavMenu>
                            ...
                            {Object.keys(navigationSetup.navigation).map(
                                function (key) {
                                    return (
                                        <Item
                                            onClick={() => {
            ...
            <ItemH justify="flex-end">
                ...
                <ItemH justify="flex-end" flex="initial">
                    ...
                    {!active && !error && (
                        <ThirdTheme>
                            Please connect to a Web3 Network
                        </ThirdTheme>
                    )}
                    <Connection
                        phase={active ? "active" : error ? "error" : "waiting"}
                    />
                </ItemH>
            </ItemH>
        </Container>
