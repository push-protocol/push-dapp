...
import { Section, Item, ItemH, AnchorLink as Anchor } from 'components/SharedStyling'
...
function Navigation(): JSX.Element {
  ...
  const { navigationSetup, setNavigationSetup } = useContext<any>(NavigationContext)
  ...
  const returnTransformedList = (lists, identity) => {
    ...
    let count = -1
    Object.entries(lists).forEach(([key, value]: any) => {
      count++
      const identifier = count.toString()
      ...
      if (section.hasOwnProperty('drilldown')) {
        let drillcount = -1
        const drilldownModified = {}

        Object.entries(section.drilldown).forEach(([drillkey, drillvalue]: any) => {
          drillcount++
          const drillIdentifier = drillcount.toString()
  ...
  const mutateTransformedList = (item, onlyDrilldown) => {
    ...
    const transformedMenuList = navigationSetup
  ...
  const renderMainItems = (items, sectionID) => {
    ...
    const rendered = Object.keys(items).map(function (key) {
      ...
      const innerRendered = (
        <Section key={key} flex="1" align="stretch" size={fontSize}>
          {secondaryButton ? (
            <Item padding="10px" flexBasis="100%" align="stretch" direction="row" overflow="hidden">
  ...
  return (
    <Container direction="column" headerHeight={GLOBALS.CONSTANTS.HEADER_HEIGHT}>
      ...
      {navigationSetup && Object.keys(navigationSetup).length > 0 && (
        <>
          <Primary>{renderMainItems(navigationSetup.primary, GLOBALS.CONSTANTS.NAVBAR_SECTIONS.PRIMARY)}</Primary>
          <Footer justify="flex-end" align="strecth">
            <Secondary align="stretch" justify="flex-end" margin="10px 0px 10px 0px">
              {renderMainItems(navigationSetup.secondary, GLOBALS.CONSTANTS.NAVBAR_SECTIONS.SECONDARY)}
            </Secondary>

            {/* Put social */}
            <ItemH flex="initial" padding="10px" radius="0px 12px 0px 0px" bg={theme.leftBarSocialBg}>
              <Anchor
                title="Open Twitter"
