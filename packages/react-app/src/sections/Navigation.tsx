...
function Navigation() {
    ...
    const { navigationSetup, setNavigationSetup } =
        useContext(NavigationContext);
    if (navigationSetup !== null && channelDetails !== null) {
        navigationSetup.primary[0].data.drilldown[3].data.name =
            channelDetails.name;
    }
    ...
    const returnTransformedList = (lists, identity) => {
        ...
        Object.entries(lists).forEach(([key, value]) => {
            ...
            if (section.hasOwnProperty("drilldown")) {
                ...
                Object.entries(section.drilldown).forEach(
                    ([drillkey, drillvalue]) => {
                        drillcount++;
                    ...
                    Object.entries(topSection).forEach(([key, value]) => {
                        ...
                        if (section.data.hasOwnProperty("drilldown")) {
                            Object.entries(section.data.drilldown).forEach(
                                ([drillkey, drillvalue]) => {
                                    const item =
                                        section.data.drilldown[drillkey];
    ...
    const mutateTransformedList = (item, onlyDrilldown) => {
        ...
        if (activeDrilldownId == null) {
            Object.keys(transformedMenuList).forEach((key) => {
                if (key === "primary" || key === "secondary") {
                    Object.keys(transformedMenuList[key]).forEach(
                        (sectionkey) => {
                            const section =
                                transformedMenuList[key][sectionkey];
                                ...
                                if (section.active) {
                                    ...
                                    if (section.hasItems) {
                                        ...
                                        Object.keys(
                                            ...
                                            transformedMenuList[key][
                                                sectionkey
                                            ].data.drilldown[drillkey].active =
                                                false;
                                        });
                                    }
                                }
                            } else {
                                transformedMenuList[key][sectionkey].active =
                                    true;
