  ...
  return (
    <Content padding="20px">
      ...
      <Item justify="flex-start" align="flex-start" margin="20px">
        <Button bg="#1C4ED8" onClick={connectToCeramic}>
          Connect to Ceramic
        </Button>
        {did && <p>{did.id}</p>}
        <ItemH>
          <label>Enter your text to be added to Ceramic:</label>

          <InputStyled type="text" value={content} onChange={e => setContent(e.target.value)} />
        </ItemH>

        <H2 textTransform="uppercase" spacing="0.1em">
          Encrypt data
        </H2>
        <Button bg="#1C4ED8" onClick={encrypt}>
          Encrypt
        </Button>
        {!encryptedData ? (
        ...
        <Button bg="#1C4ED8" onClick={decrypt}>
          Decrypt
        </Button>
        <p>{decryptedData}</p>
        ...
        <Button bg="#1C4ED8" onClick={getLinkWallets}>
          Get Links
        </Button>
