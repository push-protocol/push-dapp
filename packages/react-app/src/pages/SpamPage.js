...
function InboxPage() {
    ...
    const listenFornewNotifications = () => {
        ...
        return () =>
            epnsCommReadProvider.off.bind(epnsCommReadProvider, event, cb); //when we unmount we remove the listener
    };
    ...
    React.useEffect(() => {
        (async function init() {
            ...
            if (!!(library && account)) {
                ...
                dispatch(
                    setCommunicatorWriteProvider(communicatorSignerInstance)
                );
            }
        })();
    }, [account, chainId]);
    ...
    const checkUserForChannelOwnership = async () => {
        ...
        EPNSCoreHelper.getChannelJsonFromUserAddress(
            ownerAccount,
            epnsReadProvider
        )
            .then(async (response) => {
                ...
                const channelJson = await epnsWriteProvider.channels(
                    ownerAccount
                );
                const channelSubscribers =
                    await ChannelsDataStore.instance.getChannelSubscribers(
