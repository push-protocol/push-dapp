...
function YieldFarmingPage() {
    ...
    const getLPPoolStats = React.useCallback(
        async (poolStats) => {
            const lpPoolStats =
                await YieldFarmingDataStore.instance.getLPPoolStats(poolStats);
    ...
    const getFormattedDuration = () => {
        if (poolStats?.epochEndTimestamp) {
            ...
            const minutes = parseInt(
                (duration - (day * 86400 + hour * 3600)) / 60
            );
            const seconds =
                duration - (day * 86400 + hour * 3600 + minutes * 60);
                                ...
                                <StatsCard bg={themes.mainBg}>
                                    ...
                                    <StatsContent>
                                        <StatsInnerTitle>{`$ ${numberWithCommas(
                                            poolStats.totalValueLocked.toFixed(
                                                2
                                            )
                                        )}`}</StatsInnerTitle>
                                    </StatsContent>
                                ...
                                <StatsCard bg={themes.mainBg}>
                                    ...
                                    <StatsContent>
                                        <StatsInnerTitle>
                                            {numberWithCommas(
                                                formatTokens(
                                                    poolStats.pushRewardsDistributed
                                                )
                                            )}
                                        </StatsInnerTitle>
                                        <StatsInnerSub>
                                            ...
                                            {numberWithCommas(
                                                formatTokens(
                                                    poolStats.totalDistributedAmount
                                                )
                                            )}
                                        </StatsInnerSub>
                                    </StatsContent>
                                    <StatsPreview color="#35c5f3">
                                        Rewarded
                                    </StatsPreview>
                                </StatsCard>

                                <StatsCard bg={themes.mainBg}>
                                    <StatsHeading bg="#674c9f">
                                        Time Left
                                    </StatsHeading>
                                    <StatsContent>
                                        <StatsInnerTitle>
