...
function InboxPage() {
  ...
  const enableSecretNotif = async () => {
    if (!epnsCommReadProvider) return;
        ...
        <Item>
          <Item margin="16px 20px 0px 0px" self="self-end">
            <Button
              ...
              bg={themes.scheme === "light" ? GLOBALS.COLORS.GRADIENT_PRIMARY : GLOBALS.COLORS.GRADIENT_SECONDARY}
              radius="50px"
              onClick={enableSecretNotif}
            >
              <span style={{ color: "#fff" }}>Enable Secret Notifications</span>
              <></>
            </Button>
          </Item>
        </Item>
