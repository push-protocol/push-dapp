...
export default class YieldFarmingDataStore {
    ...
    getPoolStats = () => {
        return new Promise(async (resolve, reject) => {
            ...
            const pushPriceAmounts =
                await this.state.uniswapV2Router02.getAmountsOut(
                    ONE_PUSH.toString(),
                    [
                        addresses.epnsToken,
            ...
            const pushPrice =
                pushPriceAmounts[pushPriceAmounts.length - 1].toNumber() /
                1000000;
            ...
            const ethPriceAmounts =
                await this.state.uniswapV2Router02.getAmountsOut(
                    ONE_PUSH.toString(),
                    [addresses.WETHAddress, addresses.USDTAddress]
                );
            const ethPrice =
                ethPriceAmounts[ethPriceAmounts.length - 1].toNumber() /
                1000000;

            const uniTotalSupply = tokenBNtoNumber(
                await this.state.epnsToken
                    .attach(addresses.epnsLPToken)
                    .totalSupply()
            ...
            const start = epochStart.add(
                currentEpochPUSH.sub(1).mul(epochDuration)
            );
    ...
    getPUSHPoolStats = async () => {
        return new Promise(async (resolve, reject) => {
            ...
            const currentEpochPUSH = await yieldFarmingPUSH.getCurrentEpoch();
            const totalEpochPUSH = (
                await yieldFarmingPUSH.NR_OF_EPOCHS()
            ).toString();
    ...
    getLPPoolStats = async (poolStats) => {
        return new Promise(async (resolve, reject) => {
            ...
            const totalEpochPUSH = (
                await yieldFarmingLP.NR_OF_EPOCHS()
            ).toString();
            const genesisEpochAmount = tokenToBn(
        ...
        return new Promise(async (resolve, reject) => {
            if (this.state.account) {
                ...
                const potentialUserReward = (
                    await this.calculateUserEpochReward(
                        currentEpochPUSH,
                        contract
                    )
                ).toFixed(2);
                ...
                for (var i = 0; i <= currentEpochPUSH.sub(1).toNumber(); i++) {
                    const epochReward = await this.calculateUserEpochReward(
                        i,
                        contract
                    );
                    totalAccumulatedReward =
                        totalAccumulatedReward + epochReward;
                }
    ...
    calculateUserEpochReward = async (epochId, contract) => {
        ...
        if (poolSize > 0) {
            if (contract.address == addresses.yieldFarmLP) {
                ...
                potentialUserReward =
                    (epochStake / poolSize) * rewardForCurrentEpoch;
            } else {
                const rewardForCurrentEpoch = tokenBNtoNumber(
