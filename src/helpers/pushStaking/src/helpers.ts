...
export const Helpers = {
  ...
  getCoreV2Contract: (provider: ethers.providers.JsonRpcProvider, address: string) => {
    return PushCoreV2__factory.connect(address, provider);
  },

  lastEpochRelative: (genesisBlock: number, currentBlock: number) => {
    const calcEpoch = Math.floor((currentBlock - genesisBlock) / Constants.epochDuration + 1);
  ...
  epochToUserStakedWeight: async (
    ...
    const eTUSWtMappingSlot = ethers.BigNumber.from(userFessInfoSlotHash).add(4).toHexString();
