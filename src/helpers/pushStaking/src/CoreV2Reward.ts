...
export class CoreV2Reward {
  ...
  public async initState() {
    ...
    const genesisEpoch = await coreV2Contract.genesisEpoch().then((res) => res.toNumber());

    const currentEpoch = Helpers.lastEpochRelative(genesisEpoch, currentBlockNumber);
    ...
    const epochRewards = await Promise.all(epochs.map((ep) => coreV2Contract.epochRewards(ep)));
    const epochToTotalStakedWeight = await Promise.all(epochs.map((ep) => coreV2Contract.epochToTotalStakedWeight(ep)));
    ...
    this.STATE.previouslySetEpochRewards = await coreV2Contract.previouslySetEpochRewards();
  ...
  private async _setupEpochsRewardAndWeights(_userWeight: number) {
    ...
    const _lastEpochInitiliazed = Helpers.lastEpochRelative(genesisEpoch, this.STATE.lastEpochInitialized);

    const _currentEpoch = this.STATE.currentEpoch;

    // Setting up Epoch Based Rewards
    console.log('\n\nDoing with: _currentEpoch, _lastEpochInitiliazed', _currentEpoch, _lastEpochInitiliazed, '\n');

    if (_currentEpoch > _lastEpochInitiliazed || _currentEpoch == 1) {
      const PROTOCOL_POOL_FEES = await coreV2Contract.PROTOCOL_POOL_FEES();
      const previouslySetEpochRewards = await coreV2Contract.previouslySetEpochRewards();
      ...
      if (_epochGap > 1) {
        this.STATE.epochRewards[_currentEpoch - 1] =
          this.STATE.epochRewards[_currentEpoch - 1].add(availableRewardsPerEpoch);
      } else {
        this.STATE.epochRewards[_currentEpoch] = Helpers.toBN(this.STATE.epochRewards[_currentEpoch].toString()).add(
          availableRewardsPerEpoch
        );
      }

      this.STATE.lastEpochInitialized = await coreV2Contract.provider.getBlockNumber();
      this.STATE.previouslySetEpochRewards = PROTOCOL_POOL_FEES;
    }

    // Setting up Epoch Based TotalWeight
    if (this.STATE.lastTotalStakeEpochInitialized == 0 || this.STATE.lastTotalStakeEpochInitialized == _currentEpoch) {
      this.STATE.epochToTotalStakedWeight[_currentEpoch] =
        this.STATE.epochToTotalStakedWeight[_currentEpoch].add(_userWeight);
    } else {
      for (let i = this.STATE.lastTotalStakeEpochInitialized + 1; i <= _currentEpoch - 1; i++) {
        if (this.STATE.epochToTotalStakedWeight[i].toNumber() == 0) {
          this.STATE.epochToTotalStakedWeight[i] =
            this.STATE.epochToTotalStakedWeight[this.STATE.lastTotalStakeEpochInitialized];
        }
      }

      this.STATE.epochToTotalStakedWeight[_currentEpoch] =
        this.STATE.epochToTotalStakedWeight[this.STATE.lastTotalStakeEpochInitialized].add(_userWeight);
    }
  ...
  private async _adjustUserAndTotalStake(_userWeight: number) {
    ...
    if (userStakedWeight.isZero()) {
      ...
      if (currentEpoch == lastStakedEpoch) {
        this.STATE.userFeesInfo.stakedWeight = userStakedWeight.add(_userWeight);
      } else {
        // Initiating 2.2 Case: User stakes again but in Different Epoch
        for (let i = lastStakedEpoch; i <= currentEpoch; i++) {
          if (i != currentEpoch) {
            this.STATE.userFeesInfo.epochToUserStakedWeight[i] = userStakedWeight;
          } else {
            this.STATE.userFeesInfo.stakedWeight = userStakedWeight.add(_userWeight);

            this.STATE.userFeesInfo.epochToUserStakedWeight[i] = this.STATE.userFeesInfo.stakedWeight;
          }
        }
      }
    }
    ...
    if (_userWeight != 0) {
      this.STATE.userFeesInfo.lastStakedBlock = Helpers.toBN(this.STATE.currentBlockNumber);
    }
  }
  ...
  public async getUserPotentialEpochReward() {
    ...
    const totalStakedWeight = this.STATE.epochToTotalStakedWeight[this.STATE.currentEpoch];
    const userstakedWeight = this.STATE.userFeesInfo.stakedWeight;
    const potentialReward = userstakedWeight.mul(epochRewards).div(totalStakedWeight);
