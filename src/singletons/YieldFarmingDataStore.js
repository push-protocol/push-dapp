...
export default class YieldFarmingDataStore {
    ...
    return new Promise(async (resolve, reject) => {
      if (this.state.account) {
        const currentEpochPUSH = await contract
          .getCurrentEpoch()
          .then((res) => ethers.BigNumber.from(Math.min(res, 100))); //100

        const epoch = await contract.getCurrentEpoch(); // 120

        const epochStakeNext = await contract.getEpochStake(this.state.account, epoch.add(1));
  ...
  getTotalAvailableRewards = async (lastEpochIdHarvested, currentEpochPUSH, contract) => {
    let promises = [];
  ...
  getAccumulatedReward = async (currentEpochPUSH, contract) => {
    let promises = [];
