...
function ViewDelegateeItem({ delegateeObject, epnsToken, signerObject, pushBalance, setGaslessInfo, theme }) {
    ...
    const importImages = async () => {
      try {
        const [imageModule, image2xModule, image3xModule] = await Promise.all([
          import(`../assets/gov/delegatees/${delegateeObject.pic}.jpg`),
          import(`../assets/gov/delegatees/${delegateeObject.pic}@2x.jpg`),
          import(`../assets/gov/delegatees/${delegateeObject.pic}@3x.jpg`),
        ]);

        setImageSrc(imageModule.default);
        setImageSrcSet(`${image2xModule.default} 2x, ${image3xModule.default} 3x`);
      } catch (error) {
  ...
  const delegateAction = async (delegateeAddress) => {
    ...
    if (!web3.utils.isAddress(delegateeAddress)) delegateeAddress = await ens.getAddress(delegateeAddress);
          ...
          <DelegateeImageInner>
            ...
            {!loading && !delegateeObject.pic && (
              <Blockies
                ...
                opts={{ seed: delegateeObject.wallet.toLowerCase(), size: 30, scale: 10 }}
              />
            )}
          </DelegateeImageInner>
