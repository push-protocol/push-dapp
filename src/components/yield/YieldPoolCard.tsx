...
const YieldPoolCard = ({
  ...
  const massClaimRewardsTokensAll = async () => {
    ...
    let yieldFarming = new ethers.Contract(poolAddress, abis.yieldFarming, signer);
  ...
  const withdrawTokens = async () => {
    ...
    const amounttowithdraw = await staking.balanceOf(account, tokenAddress);
    const tx = staking.withdraw(
      tokenAddress,
      ethers.BigNumber.from(withdrawAmount).mul(ethers.BigNumber.from(10).pow(18))
    );
    ...
    tx.then(async (tx) => {
      ...
      yieldFarmToast.showMessageToast({
        toastTitle: 'Error',
        toastMessage: `Transaction Cancelled! ${err}`,
        toastType: 'ERROR',
        getToastIcon: (size) => (
          <MdError
            size={size}
  ...
  const migrateToNewPool = async () => {
    ...
    tx = staking.withdraw(tokenAddress, ethers.BigNumber.from(withdrawAmount).mul(ethers.BigNumber.from(10).pow(18)));
  ...
  const depositLpToken = async (tx, withdrawAmount, totalTxnSteps) => {
    ...
    tx = stakingV2.deposit(tokenAddress, ethers.BigNumber.from(withdrawAmount).mul(ethers.BigNumber.from(10).pow(18)));
    tx.then(async (tx) => {
  ...
  const depositPushToken = async (tx, withdrawAmount, totalTxnSteps) => {
    ...
    tx = pushCoreV2.stake(ethers.BigNumber.from(withdrawAmount).mul(ethers.BigNumber.from(10).pow(18)));
  ...
  return (
    <Container margin={poolName === 'UNI-V2' ? ' 10px 0 10px 10px ' : '  10px 10px 10px 0'}>
      {/* Top Section */}
      <ItemVV2 margin="0px 0px 20px 0">
        {PoolStats ? (
          ...
          <SkeletonContainer padding="15px 15px 0 15px">
            <SkeletonLine
              height="15px"
              width="234px"
              margin="0 0 10px 0"
        ...
        >
          {PoolStats ? (
            <>
              ...
              <EpochNo margin="0 0 0 5px">100 / 100</EpochNo>
            </>
          ) : (
          ...
          {userData ? (
            <ItemVV2>
              ...
              >
                ...
                <DataValue>
                  {' '}
                  {formatTokens(userData?.epochStakeNext)} {poolName == 'UNI-V2' ? 'UNI-V2' : 'PUSH'}
                </DataValue>
              </ItemHV2>
              <ItemHV2
                justifyContent="space-between"
              ...
              >
                ...
                <DataValue>
                  {' '}
                  {numberWithCommas(
                    (userData?.totalAccumulatedReward - userData?.totalAvailableReward).toFixed(2)
                  )}{' '}
                  PUSH
                </DataValue>
              </ItemHV2>
                    ...
                    <Text>
                      {!txInProgressMigrate && MigrateMessage == null && (
                        <Span
                          ...
                          cursor="default"
                        >
                          Migrate to {poolName === 'UNI-V2' ? 'UNI-V2 ' : 'PUSH Fee'} Pool
                        </Span>
                      )}
                    </Text>
                  </EmptyButton>
