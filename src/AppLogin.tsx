                    ...
                    try {
                      ...
                      if (currentConnector instanceof WalletConnect) {
                        ...
                        if (window.ethereum === undefined || window.ethereum.networkVersion === undefined) {
                          setAuthError({
                            message:
                              'Web3 not enabled, install MetaMask on desktop or visit from a dApp browser on mobile',
                          });
                        } else {
                          await currentConnector.activate(
                            chainIds.includes(parseInt(window.ethereum.networkVersion))
                              ? ''
                              : getAddChainParameters(desiredChain)
                          );
                        }
                      }
                    } catch (error) {
