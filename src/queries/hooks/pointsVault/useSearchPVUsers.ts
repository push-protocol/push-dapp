...
export const useSearchPVUsers = ({ query, pageSize = 20 }: PointsVaultSearchPayload) => {
  const token = usePointsVaultToken();

  return useInfiniteQuery({
    queryKey: [pointsVaultUsers, pointsVaultSearch, query],
    initialPageParam: 1,
    queryFn: ({ pageParam }) => getPVSearchResults({ page: pageParam as number, pageSize, query, token }),
