...
`,is=a(Ce)`
  ...
  border-radius: 5px;
  gap: 5px;
`,Tt=({onClose:t,InnerComponentProps:s,toastObject:i})=>{const{title:p,getUserData:n,getPoolStats:o,setUnstakeErrorMessage:h,setWithdrawErrorMessage:x}=s,{account:d,provider:u,isWalletConnected:v,connect:E}=ke(),[W,B]=r.useState(0),[L,$]=r.useState(0),[G,O]=r.useState(!1),[X,U]=r.useState(!1),[ae,Y]=r.useState(!1);r.useState(null),r.useContext(Ls);const[H,P]=r.useState(0),V=()=>{h(null),x(null),t()},M=_e(),w=r.useRef(null);$s(w,()=>V());const R=async()=>{var m=u.getSigner(d);const g=p==="Uni-V2"?l.uniV2LPToken:l.pushToken,b=new C(g,k.uniV2LpToken,m);let j=Ws(await b.balanceOf(d));B(parseInt(j.toString().replace(/\D/,""))||0)},K=async()=>{U(!0);var m=u.getSigner(d);let g;p==="Uni-V2"?g=await new C(l.uniV2LPToken,k.uniV2LpToken,m).allowance(d,l.stakingV2):g=await new C(l.pushToken,k.uniV2LpToken,m).allowance(d,l.pushCoreV2),$(_(g)),U(!1)};r.useEffect(()=>{R(),K()},[]);const me=async()=>{if(!v){E();return}if(G||X)return;U(!0);var m=u.getSigner(d);let g;const b=Bs(H.toString(),18);p==="Uni-V2"?g=new C(l.uniV2LPToken,k.uniV2LpToken,m).approve(l.stakingV2,b):g=new C(l.pushToken,k.uniV2LpToken,m).approve(l.pushCoreV2,b),g.then(async j=>{i.showLoaderToast({loaderMessage:"Waiting for Confirmation..."});try{await u.waitForTransaction(j.hash),i.showMessageToast({toastTitle:"Success",toastMessage:`Successfully approved ${p} Tokens!`,toastType:"SUCCESS",getToastIcon:f=>e.jsx(xe,{size:f,color:"green"})}),U(!1),O(!0)}catch(f){console.error("Error",f),i.showMessageToast({toastTitle:"Error",toastMessage:"User denied message signature.",toastType:"ERROR",getToastIcon:A=>e.jsx(D,{size:A,color:"red"})}),U(!1)}}).catch(j=>{console.error("Error",j),i.showMessageToast({toastTitle:"Error",toastMessage:"User denied message signature",toastType:"ERROR",getToastIcon:f=>e.jsx(D,{size:f,color:"red"})}),U(!1)})},F=async()=>{if(!v){E();return}if(ae||!G)return;if(Y(!0),H==0){i.showMessageToast({toastTitle:"Error",toastMessage:`You need to deposit atleast 1 ${p} token `,toastType:"ERROR",getToastIcon:b=>e.jsx(D,{size:b,color:"red"})}),Y(!1);return}var m=u.getSigner(d);let g;p==="Uni-V2"?g=new C(l.stakingV2,k.stakingV2,m).deposit(l.uniV2LPToken,I.from(H).mul(I.from(10).pow(18))):g=new C(l.pushCoreV2,k.pushCoreV2,m).stake(I.from(H).mul(I.from(10).pow(18))),g.then(async b=>{i.showLoaderToast({loaderMessage:"Waiting for Confirmation..."});try{await u.waitForTransaction(b.hash),i.showMessageToast({toastTitle:"Success",toastMessage:"Transaction Completed!",toastType:"SUCCESS",getToastIcon:j=>e.jsx(xe,{size:j,color:"green"})}),o(),n(),Y(!1),V()}catch(j){console.error("Error",j),i.showMessageToast({toastTitle:"Error",toastMessage:`Transaction Failed! (" +${j.name}+ ")`,toastType:"ERROR",getToastIcon:f=>e.jsx(D,{size:f,color:"red"})}),Y(!1)}}).catch(b=>{console.error("Error in depositing",b),b.reason=b.reason.slice(b.reason.indexOf("::")+1),i.showMessageToast({toastTitle:"Error",toastMessage:`Transaction Cancelled! ${b.reason}`,toastType:"ERROR",getToastIcon:j=>e.jsx(D,{size:j,color:"red"})}),Y(!1)})},q=m=>{m.preventDefault(),P(parseInt(m.target.value.replace(/\D/,""))||0),L>=parseInt(m.target.value.replace(/\D/,""))?O(!0):O(!1)},we=m=>{P(parseInt(m.toString().replace(/\D/,""))||0)},S=Ee(600);return e.jsxs(as,{children:[e.jsxs(T,{justifyContent:"space-between",children:[e.jsx(os,{children:p==="Uni-V2"?"Uniswap V2 Staking Pool":"Push Fee Staking Pool"}),e.jsx(it,{onClick:()=>V(),style:{cursor:"pointer"}})]}),e.jsxs(y,{children:[e.jsx(Vs,{weight:"500",size:"14px",self:"baseline",color:M.stakingSecondaryText,children:"You are Staking"}),e.jsxs(T,{width:"-webkit-fill-available",background:M.default.bg,height:"60px",padding:S?"8px":"14px",borderRadius:"12px",border:`1px solid ${M.modalBorderColor}`,children:[e.jsx(ns,{placeholder:"Enter Amount",flex:"2",radius:"4px",size:"32px",height:"32px",self:"auto",bg:"#FFF",color:M.stakingSecondaryText,value:H,onChange:m=>{m.preventDefault(),q(m)},autoFocus:!0}),e.jsxs(rs,{onClick:()=>we(W),children:["Max: ",W]})]})]}),e.jsxs(T,{margin:"20px 0",gap:"12px",children:[e.jsxs(ie,{variant:"primary",size:"medium",onClick:me,disabled:!!G,children:[!G&&!X&&e.jsxs(Ge,{color:"#FFFFFF",weight:"400",cursor:"pointer",children:["Approve ",p]}),X&&!G&&e.jsx(le,{type:pe.SEAMLESS,spinnerSize:26,spinnerColor:"#fff",title:"Approving",titleColor:"#FFF"}),G&&e.jsx(Ge,{color:M.emptyButtonText,weight:"600",cursor:"default",children:"Approved"})]}),e.jsxs(ie,{variant:"outline",size:"medium",disabled:!!(!G||ae),onClick:F,children:[!ae&&"Deposit",ae&&e.jsx(le,{type:pe.SEAMLESS,spinnerSize:26,spinnerColor:"#FFFFFF",title:"Depositing",titleColor:"#FFF"})]})]})]})},as=a.div`
  padding: 16px 20px;
  width: 340px;
  font-family: 'FK Grotesk Neu';
  font-style: normal;
...
`,Ae=a(Ce)`
  ...
  border-radius: 5px;
  gap: 5px;
`,ys=({setActiveTab:t})=>{const{provider:s,account:i,chainId:p}=ke(),[n,o]=r.useState(),[h,x]=r.useState(),[d,u]=r.useState(),[v,E]=r.useState(),[W,B]=r.useState(),[L,$]=r.useState(null),[G,O]=r.useState(null),[X,U]=r.useState(null),[ae,Y]=r.useState(null),[H,P]=r.useState(null),V=s==null?void 0:s.getSigner(i),M=r.useCallback(async()=>{const F=await Re.getInstance().getPoolStats(s);$({...F})},[h,n,v,d,W,s]),w=r.useCallback(async()=>{const F=await Re.getInstance().getPoolStats(s),q=await Re.getInstance().getLPPoolStats(F);O({...q})},[h,n,v,d,W,s]),R=r.useCallback(async()=>{},[]),K=r.useCallback(async()=>{const F=await Re.getInstance().getUserDataLP();U({...F})},[h,n,v,d,W]),me=r.useCallback(async()=>{const[F,q]=await Re.getInstance().getUserDataPUSH(s);P({...F}),Y({...q})},[h,n,v,d,W,s]);return r.useEffect(()=>{if(p!==he.coreContractChain&&p!==he.mainnetCoreContractChain)return;O(null),U(null),P(null),Y(null);let F=new C(l.stakingV2,k.stakingV2,V),q=new C(l.pushToken,k.pushToken,V),we=new C(l.pushCoreV2,k.pushCoreV2,V),S=new C(l.yieldFarmLP,k.yieldFarming,V),m=new C(l.uniswapV2Router02,k.uniswapV2Router02,V);if(x(F),o(q),E(we),u(S),B(m),V&&i){var g=s==null?void 0:s.getSigner(i);let b=new C(l.stakingV2,k.stakingV2,g),j=new C(l.pushToken,k.pushToken,g),f=new C(l.pushCoreV2,k.pushCoreV2,g),A=new C(l.yieldFarmLP,k.yieldFarming,g),z=new C(l.uniswapV2Router02,k.uniswapV2Router02,g);x(b),o(j),E(f),u(A),B(z)}Re.getInstance().init(i,F,q,we,S,m),M(),K(),w(),me()},[i,p]),e.jsxs(e.Fragment,{children:[e.jsx(pt,{logo:"announcement",title:"Staking Program has now ended.",body:"Note: While UNI-V2 LP staking pool has ended, the Push Fee Staking Pool is designed to continue its staking mechanism. However, rewards for stakers will solely come from Core smart contract fee earnings.",setActiveTab:t}),e.jsx(ts,{getLpPoolStats:w,poolStats:L,setPoolStats:$}),e.jsx(qt,{poolStats:L}),e.jsxs(Es,{children:[e.jsx(Ts,{userDataPush:ae,getUserDataPush:me,PUSHPoolstats:H,getPUSHPoolStats:R}),e.jsx(ls,{lpPoolStats:G,userDataLP:X,getLpPoolStats:w,getUserDataLP:K})]})]})},Es=a(T)`
  @media (max-width: 1300px) {
    flex-direction: column;
  }
`,Ps=({onClose:t})=>{const{chainId:s,switchChain:i}=ke(),p=()=>{const n=he.allowedNetworks[0];s!==1&&s!==11155111&&(console.info("Current Chain ID ",s),console.info("Chain Id to pass",n),i(he.coreContractChain))};return r.useEffect(()=>{(s===he.coreContractChain||s===he.mainnetCoreContractChain)&&t()},[s]),e.jsxs(vs,{children:[e.jsx(it,{style:{cursor:"pointer",alignSelf:"end"}}),e.jsxs(Rs,{children:[e.jsx(Us,{children:"Unsupported Network"}),e.jsxs(Ms,{children:["Push Yield Farm V2 is only live on Ethereum Chain.",e.jsx("br",{}),"Kindly switch to Ethereum"]})]}),e.jsx(Fs,{children:e.jsx(ie,{onClick:p,variant:"primary",size:"large",children:"Switch Network"})})]})},vs=a(y)`
  padding: 32px 36px;
  width: 445px;
`,Rs=a(y)`
  font-family: FK Grotesk Neu;
