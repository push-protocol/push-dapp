...
`,Me=i(be)`
  ...
  border-radius: 5px;
  gap: 5px;
`,Tt=({logo:t,title:s,body:o,setActiveTab:l})=>{const r=Ee(600);return e.jsxs(T,{style:{color:t==="announcement"?"white":"#333"},justifyContent:"stretch",padding:r?"12px":"16px",background:t==="announcement"?"linear-gradient(90deg, #121315 -2.55%, #2A2A39 32.62%, #8E317A 68.34%, #121315 102.97%)":"#FFF7DA",borderRadius:"16px",children:[e.jsx(oe,{width:t==="announcement"?"60px":"32px",height:t==="announcement"?"60px":"32px",src:Ft(`svg/${t}.svg`),alt:"Announcement Logo"}),e.jsxs(k,{margin:t==="announcement"?"0 0 0 16px":"0 0 0 13px",justifyContent:"center",alignItems:"flex-start",gap:t==="announcement"?"4px":"0px",children:[e.jsx(D,{color:"inherit",fontSize:r?"20px":"24px",textAlign:"left",fontWeight:500,children:s}),e.jsxs(D,{color:"inherit",fontSize:r?"12px":"16px",textAlign:"left",fontWeight:500,children:[o," ",s.includes("New V2")&&e.jsxs(e.Fragment,{children:[e.jsx(qt,{onClick:()=>l(1),children:"here"})," to migrate."]})]})]})]})},qt=i(Rt)`
  cursor: pointer;
  color: #fff;
...
`;ge.coreContractChain;const Zt=({setActiveTab:t})=>{const{account:s,provider:o}=ye(),[l,r]=n.useState(null),[a,h]=n.useState(null),[x,d]=n.useState(null),[u,v]=n.useState(null),[E,_]=n.useState(null),[B,N]=n.useState(null),[V,G]=n.useState(null),[O,ee]=n.useState(null),[F,ae]=n.useState(null),[Y,H]=n.useState(null),P=o==null?void 0:o.getSigner(s),$=n.useCallback(async()=>{const U=await Ce.getInstance().getPoolStats();N({...U})},[l,a,x,u,E]),M=n.useCallback(async()=>{const U=await Ce.getInstance().getPUSHPoolStats();G({...U})},[l,a,x,u,E]),w=n.useCallback(async U=>{const X=await Ce.getInstance().getLPPoolStats(U);ee({...X})},[l,a,x,u,E]),R=n.useCallback(async()=>{const U=await Ce.getInstance().getUserData(u);ae({...U})},[u]),K=n.useCallback(async()=>{const U=await Ce.getInstance().getUserData(x);H({...U})},[x]);n.useEffect(()=>{let U=new b(p.pushToken,y.pushToken,P),X=new b(p.staking,y.staking,P),Te=new b(p.depYieldFarmPUSH,y.yieldFarming,P),S=new b(p.depYieldFarmLP,y.yieldFarming,P),m=new b(p.uniswapV2Router02,y.uniswapV2Router02,P);if(r(U),h(X),v(Te),d(S),_(m),P&&s){var g=o.getSigner(s);let C=new b(p.pushToken,y.pushToken,g),j=new b(p.staking,y.staking,g),f=new b(p.depYieldFarmPUSH,y.yieldFarming,g),A=new b(p.depYieldFarmLP,y.yieldFarming,g),z=new b(p.uniswapV2Router02,y.uniswapV2Router02,g);r(C),h(j),v(f),d(A),_(z)}},[s]),n.useEffect(()=>{l!=null&&a!=null&&u!=null&&(Ce.getInstance().init(s,l,a,u,x,E),$())},[$]),n.useEffect(()=>{B&&we(B)},[B]);const we=async U=>{M(),w(U),R(),K()};return e.jsxs(e.Fragment,{children:[e.jsx(Tt,{logo:"WarningCircle",title:"This reward program (V1) has ended.",body:"To continue earning rewards please migrate to new pools.",setActiveTab:t}),e.jsxs(Qt,{children:[e.jsx(gt,{poolName:"PUSH",userData:F,PoolStats:V,poolAddress:p.depYieldFarmPUSH,getUserData:R,getPoolStats:M,tokenAddress:p.pushToken,setActiveTab:t}),e.jsx(gt,{poolName:"UNI-V2",userData:Y,PoolStats:O,poolAddress:p.depYieldFarmLP,getUserData:K,getPoolStats:w,tokenAddress:p.uniV2LPToken,setActiveTab:t})]})]})},Qt=i(T)`
...
`,Ls=i.div``;ge.coreContractChain;let kt,Et,Pt,lt,vt;kt=()=>{Gs.pageview("/yield");const[t,s]=n.useState(0),[o,l]=n.useState(!1),{account:r,chainId:a,switchChain:h}=ye(),x=()=>{const E=ge.allowedNetworks[0];a!==1&&a!==5&&h(E)};n.useEffect(()=>{a!==1&&a!==11155111&&(u(),x())},[a]);const{isModalOpen:d,showModal:u,ModalComponent:v}=Je();return e.jsxs(Et,{children:[e.jsx(v,{InnerComponent:Us,onConfirm:()=>{},modalPadding:"0px",modalPosition:Ke.ON_PARENT}),e.jsxs(Pt,{children:[e.jsx(lt,{isActive:!t,onClick:()=>{l(!0),s(0)},children:"Yield Farming V2"}),e.jsxs(lt,{isActive:t,onClick:()=>{l(!1),s(1)},children:["Yield Farming V1 ",e.jsx(vt,{children:"Deprecated"})]})]}),t===0&&e.jsx(Fs,{setActiveTab:s}),t===1&&e.jsx(Zt,{setActiveTab:s})]})},Et=i(We)`
	align-items: stretch;
	align-self: center;
  flex: 1;
	background: ${t=>t.theme.default.bg};
...
`,vt=i(me)`
  ...
  height: 17px;
  width: fit-content;
`,It=()=>e.jsx(qs,{children:e.jsx(kt,{})})});export{Qs as __tla,It as default};
