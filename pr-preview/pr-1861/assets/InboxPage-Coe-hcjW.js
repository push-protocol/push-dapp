...
`;t(O)`
  width: 100%;
  margin-right: 10px;
`;const me=10,Nt=({showFilter:e,setShowFilter:r,search:s,setSearch:i})=>{const n=je(),{userPushSDKInstance:a}=H(l=>l.user),c=u.useRef(null);He(c,()=>e&&r(!1));const{account:x,chainId:S,provider:g}=ke(),{subscriptionStatus:y}=H(l=>l.channels),E=oe(),[w,k]=u.useState(null),j=()=>k(null),{run:R}=H(l=>l.userJourney),{notifications:P,page:J,finishedFetching:C}=H(l=>l.spam),[D,K]=u.useState(!1),[M,I]=u.useState(!1),z=({msg:l})=>o.jsx(Ht,{children:o.jsx(Ft,{children:l})});u.useEffect(()=>{w&&j()},[w]);const ge={POLYGON_TEST_AMOY:80002,ETH_TEST_KOVAN:42,ETH_TEST_GOERLI:5,ETH_TEST_SEPOLIA:11155111,POLYGON_MAINNET:137,ETH_MAINNET:1},ue=async()=>{if(!(M||C||R||!a)){I(!0);try{const l=await a.notification.list("SPAM",{limit:me,page:J,raw:!0});let b=Z.utils.parseApiResponse(l);b.forEach((p,h)=>{p.date=l[h].epoch,p.epoch=new Date(p.date).getTime()/1e3});const d=l.map(async(p,h)=>{p.channel=l[h].sender;let f=l[h].sender;return{...p}});b=await Promise.all(d),n(fi(b)),b.length===0&&n(Qe())}catch(l){console.error(l)}finally{I(!1)}}},we=async()=>{if(!(M||D||R)){K(!0),I(!0);try{const l=await a.notification.list("SPAM",{limit:me,page:1,raw:!0});P.length||n(Ve());let b=Z.utils.parseApiResponse(l);b.forEach((p,h)=>{p.date=l[h].epoch,p.epoch=new Date(p.date).getTime()/1e3});const d=b.map(async(p,h)=>{p.channel=l[h].sender;let f=l[h].sender;return{...p}});b=await Promise.all(d),n(di({notifs:b,pageSize:me})),b.length===0&&n(Qe())}catch(l){console.error(l)}finally{K(!1),I(!1)}}};u.useEffect(()=>{a&&we()},[a]);const be=async()=>{ue(),n(Ve())},ve=l=>Number(l)===P.length-1&&!C&&!D,F=ci(),ye=async(l,b)=>{if(!l)return;let d=l;if(!d)return;console.debug(d),F.showLoaderToast({loaderMessage:"Waiting for Confirmation..."}),console.debug(g,x);const p=await g.getSigner(x);console.debug(p),console.debug({signer:p,channelAddress:ce(l,ge[b]),userAddress:ce(x,S)}),await Z.channels.subscribe({signer:p,channelAddress:ce(l,S),userAddress:ce(x,S),onSuccess:()=>{F.showMessageToast({toastTitle:"Success",toastMessage:"Successfully opted into channel !",toastType:"SUCCESS",getToastIcon:h=>o.jsx(pi,{size:h,color:"green"})}),n(xi({channelAddress:l,status:!0}))},onError:h=>{console.error(h),F.showMessageToast({toastTitle:"Error",toastMessage:"There was an error opting into channel",toastType:"ERROR",getToastIcon:f=>o.jsx(hi,{size:f,color:"red"})})},env:Ae.pushNodesEnv})},Se=l=>y[l],Ee=async({secret:l,title:b,message:d,image:p,cta:h})=>{try{let f=await _.decryptWithWalletRPCMethod(g,l,x);const m=await _.decryptWithAES(d,f);let v=await _.decryptWithAES(b,f),A=await _.decryptWithAES(p,f),L=await _.decryptWithAES(h,f);return{title:v,body:m,image:A,cta:L}}catch(f){f.code===4001?(console.error(f),$.dark(o.jsx(z,{msg:"User denied message decryption"}),{position:"bottom-right",type:$.TYPE.ERROR,autoClose:5e3,hideProgressBar:!0,closeOnClick:!0,pauseOnHover:!0,draggable:!0,progress:void 0})):f.code===-32601?(console.error(f),$.dark(o.jsx(z,{msg:"Your wallet doesn't support message decryption."}),{position:"bottom-right",type:$.TYPE.ERROR,autoClose:5e3,hideProgressBar:!0,closeOnClick:!0,pauseOnHover:!0,draggable:!0,progress:void 0})):(console.error(f),$.dark(o.jsx(z,{msg:"There was an error in message decryption"}),{position:"bottom-right",type:$.TYPE.ERROR,autoClose:5e3,hideProgressBar:!0,closeOnClick:!0,pauseOnHover:!0,draggable:!0,progress:void 0}))}};return o.jsx(Ge,{theme:E,children:o.jsx(zt,{children:o.jsxs(Lt,{children:[P&&o.jsxs(It,{id:"scrollstyle-secondary",children:[D&&o.jsx(O,{padding:"10px 20px",children:o.jsx(ae,{type:se.SEAMLESS})}),P.map((l,b)=>{const{cta:d,title:p,message:h,app:f,icon:m,image:v,secret:A,notification:L,channel:N,blockchain:G,url:Te}=l;return o.jsxs(Ut,{children:[ve(b)&&!M&&o.jsx(B,{onEnter:be}),o.jsx($e,{notificationTitle:p,notificationBody:h,cta:d,app:f,icon:m,image:v,theme:E.scheme,subscribeFn:()=>ye(N,G),isSpam:!0,isSubscribedFn:async()=>Se(N),isSecret:A!="",decryptFn:()=>Ee({secret:A,title:p,message:h,image:v,cta:d}),chainName:G,url:Te})]},b)}),M&&!D&&o.jsx(ae,{type:se.SEAMLESS})]}),!P.length&&!M&&o.jsx(Mt,{children:o.jsx(qe,{title:"You currently have no spam notifications."})}),w&&o.jsx(Be,{notification:w,clearToast:j})]})})})},Mt=t.div`
  padding: 20px;
  display: flex;
  justify-content: center;
  align-items: center;
...
`,Ft=t.div`
  margin: 0px 10px;
`,Gt=({isSpam:e})=>{const[r,s]=u.useState(!e),[i,n]=u.useState(!1),[a,c]=u.useState("");oe();const x=Fe(),S=()=>s(y=>!y),g=y=>{S(),n(!1),c(""),x(y)};return o.jsxs(Wt,{children:[o.jsx(Yt,{children:o.jsxs(Jt,{children:[o.jsx(Ie,{isActive:r,onClick:()=>g(_e.Inbox),children:"Inbox"}),o.jsx(Ie,{isActive:!r,onClick:()=>g(_e.Spam),children:"Spam"})]})}),r?o.jsx(St,{showFilter:i,setShowFilter:n,search:a,setSearch:c}):o.jsx(Nt,{showFilter:i,setShowFilter:n,search:a,setSearch:c})]})},Wt=t.div`
  height: 100%;
  width: 100%;
`,Yt=t.div`
  margin: 0px 30px;
...
`;const Kt=Ae.coreContractChain,Vt=({isSpam:e})=>{mi.pageview(_e.Inbox);const r=je(),{account:s,chainId:i,provider:n}=ke(),{epnsReadProvider:a,epnsCommReadProvider:c}=H(E=>E.contracts),[x,S]=U.useState(null),g=()=>S(null);oe();const y=Kt===i;return U.useEffect(()=>{x&&g()},[x]),U.useEffect(()=>{i&&async function(){const E=y?n:new gi(Ae.coreRPC),w=new Xe(ui.epnscore,Ze.epnscore,E),k=wi[i].commAddress,j=new Xe(k,Ze.epnsComm,n);r(bi(j)),r(vi(w))}()},[s,i]),U.useEffect(()=>{!a||!c||(a.pushChannelAdmin().then(E=>{r(yi(E))}).catch(E=>{console.error({err:E})}),a!=null&&c!=null&&(Si.getInstance().init(s,a,c),Ei.getInstance().init(s,a,c,i)))},[a,c]),o.jsx(Qt,{children:o.jsxs(Xt,{children:[o.jsx("div",{className:"joyride"}),o.jsx(Gt,{isSpam:e}),x&&o.jsx(Be,{notification:x,clearToast:g})]})})},Qt=t(Ti)`
  align-items: stretch;
  align-self: stretch;
  flex: 1;
  background: ${e=>e.theme.default.bg};
  ...
  height: calc(
    100vh - ${T.CONSTANTS.HEADER_HEIGHT}px - ${Q.BIG_MODULES.DESKTOP.TOP} -
      ${Q.BIG_MODULES.DESKTOP.BOTTOM}
  );

  @media ${V.laptop} {
    margin: ${T.ADJUSTMENTS.MARGIN.BIG_MODULES.TABLET};
    height: calc(
      100vh - ${T.CONSTANTS.HEADER_HEIGHT}px - ${Q.BIG_MODULES.TABLET.TOP} -
        ${Q.BIG_MODULES.TABLET.BOTTOM}
    );
  ...
  @media ${V.mobileL} {
    ...
    height: calc(
      100vh - ${T.CONSTANTS.HEADER_HEIGHT}px - ${Q.BIG_MODULES.MOBILE.TOP} -
        ${Q.BIG_MODULES.MOBILE.BOTTOM}
    );
