...
`;const Kt=_e.coreContractChain,Vt=({isSpam:e})=>{mi.pageview(Ae.Inbox);const r=je(),{account:s,chainId:i,provider:n}=ke(),{epnsReadProvider:a,epnsCommReadProvider:d}=H(E=>E.contracts),[x,S]=z.useState(null),g=()=>S(null);oe();const y=Kt===i;return z.useEffect(()=>{x&&g()},[x]),z.useEffect(()=>{i&&async function(){const E=y?n:new gi(_e.coreRPC),w=new Qe(ui.epnscore,Ze.epnscore,E),k=wi[i].commAddress,j=new Qe(k,Ze.epnsComm,n);r(bi(j)),r(vi(w))}()},[s,i]),z.useEffect(()=>{!a||!d||a.pushChannelAdmin().then(E=>{r(yi(E))}).catch(E=>{console.error({err:E})})},[a,d]),o.jsx(Xt,{children:o.jsxs(Qt,{children:[o.jsx("div",{className:"joyride"}),o.jsx(Gt,{isSpam:e}),x&&o.jsx(Be,{notification:x,clearToast:g})]})})},Xt=t(Si)`
  align-items: stretch;
  align-self: stretch;
  flex: 1;
  background: ${e=>e.theme.default.bg};
