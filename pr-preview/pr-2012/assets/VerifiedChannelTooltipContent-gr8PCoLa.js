import { ea as e, er as N, gh as _, eQ as U, fe as D, h0 as F, h1 as O, f_ as E, h2 as q, h3 as B, e7 as Z, eb as i, ep as h, gS as z, h4 as G, h5 as Q, h6 as W, eq as J, ev as Y, e5 as X, h7 as $, ga as e2, h8 as s2, gf as j, gR as V, gc as I, fR as t2, en as v, h9 as P, ha as n2, hb as r2, hc as o2, hd as a2, __tla as __tla_0 } from "./index-UiBHnyIz.js";
import { P as i2, __tla as __tla_1 } from "./PushLogo-Bp-8LZIY.js";
let f2, L2, b2, S2, m2;
let __tla = Promise.all([
  (() => {
    try {
      return __tla_0;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_1;
    } catch {
    }
  })()
]).then(async () => {
  let c2, l2, d2, u2, p2, C2, h2;
  f2 = (r) => {
    const { svgProps: n, ...o } = r;
    return e.jsx(N, {
      componentName: "NotificationMobile",
      icon: e.jsxs("svg", {
        width: "inherit",
        height: "inherit",
        viewBox: "0 0 32 32",
        fill: "none",
        xmlns: "http://www.w3.org/2000/svg",
        ...n,
        children: [
          e.jsx("path", {
            d: "M13.0551 8.7841L13.3261 9.48341L13.6492 9.35821L13.7632 9.03109L13.0551 8.7841ZM18.949 8.79549L18.2397 9.03943L18.352 9.36576L18.6728 9.49281L18.949 8.79549ZM25.0106 22.1589L24.3706 22.5503L24.3774 22.5609L25.0106 22.1589ZM25.229 22.8955L24.479 22.9004V22.9004L25.229 22.8955ZM25.0203 23.6345L24.382 23.2406L24.3786 23.2462L25.0203 23.6345ZM24.4518 24.1735L24.0932 23.5148H24.0932L24.4518 24.1735ZM23.6735 24.3677L23.678 23.6177H23.6735V24.3677ZM8.3264 24.3677V23.6177L8.32189 23.6177L8.3264 24.3677ZM7.54814 24.1735L7.90676 23.5148H7.90676L7.54814 24.1735ZM6.97966 23.6345L7.62132 23.2462L7.6179 23.2406L6.97966 23.6345ZM6.7709 22.8955L7.52089 22.9004L6.7709 22.8955ZM6.98934 22.1589L7.62261 22.561L7.6292 22.5502L6.98934 22.1589ZM8.24889 15.5693H8.99889L8.99889 15.5683L8.24889 15.5693ZM8.83631 12.7621L8.1491 12.4617L8.1491 12.4617L8.83631 12.7621ZM10.5163 10.3821L11.0317 10.927L10.5163 10.3821ZM13.0323 8.79298L13.3054 9.49146L13.3055 9.49145L13.0323 8.79298ZM13.7632 9.03109C14.062 8.17457 14.9339 7.52087 16 7.52087V6.02087C14.3207 6.02087 12.8638 7.05503 12.3469 8.53711L13.7632 9.03109ZM16 7.52087C17.0697 7.52087 17.9437 8.17883 18.2397 9.03943L19.6582 8.55155C19.1458 7.06174 17.6848 6.02087 16 6.02087V7.52087ZM24.501 15.6518C24.501 12.2347 22.2992 9.31545 19.2251 8.09817L18.6728 9.49281C21.2272 10.5043 23.001 12.9044 23.001 15.6518H24.501ZM24.501 16.3025V15.6518H23.001V16.3025H24.501ZM25.6504 21.7677C25.1636 20.9715 24.501 19.3348 24.501 16.3025H23.001C23.001 19.5574 23.7143 21.4767 24.3707 22.5502L25.6504 21.7677ZM25.979 22.8905C25.9764 22.488 25.8592 22.0963 25.6438 21.757L24.3774 22.5609C24.4445 22.6666 24.4783 22.7838 24.479 22.9004L25.979 22.8905ZM25.6585 24.0284C25.8697 23.6863 25.9817 23.2931 25.979 22.8905L24.479 22.9004C24.4798 23.017 24.4476 23.1344 24.382 23.2406L25.6585 24.0284ZM24.8104 24.8322C25.1602 24.6418 25.4555 24.3639 25.6619 24.0228L24.3786 23.2462C24.3137 23.3534 24.2167 23.4475 24.0932 23.5148L24.8104 24.8322ZM23.669 25.1177C24.0671 25.1201 24.4608 25.0225 24.8104 24.8322L24.0932 23.5148C23.9695 23.5821 23.826 23.6186 23.678 23.6177L23.669 25.1177ZM8.3264 25.1177H23.6735V23.6177H8.3264V25.1177ZM7.18953 24.8322C7.5391 25.0225 7.93285 25.1201 8.33092 25.1177L8.32189 23.6177C8.1739 23.6186 8.03047 23.5821 7.90676 23.5148L7.18953 24.8322ZM6.33802 24.0228C6.54443 24.3639 6.83974 24.6418 7.18953 24.8322L7.90676 23.5148C7.78326 23.4475 7.68619 23.3534 7.62129 23.2462L6.33802 24.0228ZM6.02092 22.8905C6.01828 23.2931 6.13027 23.6863 6.34141 24.0284L7.6179 23.2406C7.55232 23.1344 7.52012 23.017 7.52089 22.9004L6.02092 22.8905ZM6.35617 21.757C6.1407 22.0963 6.02356 22.488 6.02092 22.8905L7.52089 22.9004C7.52165 22.7838 7.5554 22.6666 7.62252 22.5609L6.35617 21.757ZM7.49889 16.3025C7.49889 19.3348 6.83633 20.9715 6.34949 21.7677L7.6292 22.5502C8.28564 21.4767 8.99889 19.5574 8.99889 16.3025H7.49889ZM7.49889 15.5693V16.3025H8.99889V15.5693H7.49889ZM8.1491 12.4617C7.71884 13.446 7.49748 14.5025 7.49889 15.5703L8.99889 15.5683C8.99776 14.7098 9.17563 13.8583 9.52353 13.0624L8.1491 12.4617ZM10.0009 9.83727C9.20931 10.5861 8.5793 11.4775 8.1491 12.4617L9.52353 13.0624C9.87148 12.2664 10.3832 11.5404 11.0317 10.927L10.0009 9.83727ZM12.7591 8.09451C11.73 8.497 10.7924 9.08857 10.0009 9.83727L11.0317 10.927C11.6802 10.3135 12.4525 9.82505 13.3054 9.49146L12.7591 8.09451ZM12.784 8.08478C12.7757 8.08802 12.7674 8.09126 12.759 8.09451L13.3055 9.49145C13.3123 9.48876 13.3192 9.48608 13.3261 9.48341L12.784 8.08478Z",
            fill: "currentColor"
          }),
          e.jsx("path", {
            d: "M19.356 24.3901C19.356 26.2436 17.8535 27.7462 16 27.7462C14.1465 27.7462 12.644 26.2436 12.644 24.3901",
            stroke: "currentColor",
            "stroke-width": "1.5",
            "stroke-linecap": "round"
          }),
          e.jsx("path", {
            d: "M21.873 4.25385C23.4932 5.28763 24.8073 6.73605 25.679 8.44889",
            stroke: "currentColor",
            "stroke-width": "1.5",
            "stroke-linecap": "round",
            "stroke-linejoin": "round"
          }),
          e.jsx("path", {
            d: "M6.32111 8.44889C7.1928 6.73605 8.50689 5.28763 10.1271 4.25385",
            stroke: "currentColor",
            "stroke-width": "1.5",
            "stroke-linecap": "round",
            "stroke-linejoin": "round"
          })
        ]
      }),
      ...o
    });
  };
  L2 = (r) => {
    const { svgProps: n, ...o } = r;
    return e.jsx(N, {
      componentName: "TickDecoratedCircleFilled",
      icon: e.jsx("svg", {
        width: "inherit",
        height: "inherit",
        viewBox: "0 0 16 16",
        fill: "none",
        xmlns: "http://www.w3.org/2000/svg",
        ...n,
        children: e.jsx("path", {
          id: "Vector",
          d: "M14.9929 6.2C14.7214 5.92143 14.4429 5.62857 14.3357 5.37857C14.2286 5.12857 14.2357 4.75714 14.2286 4.37857C14.2214 3.68571 14.2071 2.89286 13.6571 2.34286C13.1071 1.79286 12.3143 1.77857 11.6214 1.77143C11.2429 1.76429 10.8571 1.75714 10.6214 1.66429C10.3857 1.57143 10.0786 1.27857 9.8 1.00714C9.30714 0.535714 8.74286 0 8 0C7.25714 0 6.69286 0.535714 6.2 1.00714C5.92143 1.27857 5.62857 1.55714 5.37857 1.66429C5.12857 1.77143 4.75714 1.76429 4.37857 1.77143C3.68571 1.77857 2.89286 1.79286 2.34286 2.34286C1.79286 2.89286 1.77857 3.68571 1.77143 4.37857C1.76429 4.75714 1.75714 5.14286 1.66429 5.37857C1.57143 5.61429 1.27857 5.92143 1.00714 6.2C0.535714 6.69286 0 7.25714 0 8C0 8.74286 0.535714 9.30714 1.00714 9.8C1.27857 10.0786 1.55714 10.3714 1.66429 10.6214C1.77143 10.8714 1.76429 11.2429 1.77143 11.6214C1.77857 12.3143 1.79286 13.1071 2.34286 13.6571C2.89286 14.2071 3.68571 14.2214 4.37857 14.2286C4.75714 14.2357 5.14286 14.2429 5.37857 14.3357C5.61429 14.4286 5.92143 14.7214 6.2 14.9929C6.69286 15.4643 7.25714 16 8 16C8.74286 16 9.30714 15.4643 9.8 14.9929C10.0786 14.7214 10.3714 14.4429 10.6214 14.3357C10.8714 14.2286 11.2429 14.2357 11.6214 14.2286C12.3143 14.2214 13.1071 14.2071 13.6571 13.6571C14.2071 13.1071 14.2214 12.3143 14.2286 11.6214C14.2357 11.2429 14.2429 10.8571 14.3357 10.6214C14.4286 10.3857 14.7214 10.0786 14.9929 9.8C15.4643 9.30714 16 8.74286 16 8C16 7.25714 15.4643 6.69286 14.9929 6.2ZM11.5357 6.7L7.35 10.7C7.24179 10.8018 7.09856 10.8581 6.95 10.8571C6.80362 10.8577 6.66276 10.8014 6.55714 10.7L4.46429 8.7C4.40624 8.64936 4.35903 8.5875 4.3255 8.51814C4.29197 8.44879 4.2728 8.37337 4.26916 8.29642C4.26552 8.21947 4.27748 8.14258 4.30431 8.07037C4.33115 7.99816 4.3723 7.93212 4.42531 7.87622C4.47832 7.82032 4.54208 7.77572 4.61277 7.74509C4.68345 7.71446 4.7596 7.69844 4.83663 7.69799C4.91367 7.69755 4.99 7.71268 5.06103 7.74249C5.13207 7.77229 5.19635 7.81615 5.25 7.87143L6.95 9.49286L10.75 5.87143C10.8611 5.77449 11.0054 5.72426 11.1527 5.73123C11.3 5.73819 11.4389 5.80183 11.5403 5.90882C11.6418 6.01581 11.6979 6.15789 11.6971 6.30533C11.6962 6.45278 11.6384 6.59419 11.5357 6.7Z",
          fill: "currentColor"
        })
      }),
      ...o
    });
  };
  c2 = (r) => r;
  l2 = (r, n) => r.notification.subscriptions({
    channel: n
  }).then(c2);
  d2 = async ({ signer: r, channelAddress: n, userAddress: o, env: l, decryptedPGPKeys: d }) => await _.channels.unsubscribeV2({
    signer: r,
    channelAddress: n,
    userAddress: o,
    env: l,
    pgpPrivateKey: d
  });
  u2 = async ({ userPushSDKInstance: r, channelAddress: n, settings: o }) => await r.notification.subscribe(n, {
    settings: o
  });
  m2 = (r, n) => {
    const { userPushSDKInstance: o } = U((c) => c.user);
    return D({
      queryKey: [
        r ? F : O,
        o == null ? void 0 : o.account,
        r || null
      ],
      queryFn: () => l2(o, r),
      ...n
    });
  };
  p2 = () => E({
    mutationKey: [
      q
    ],
    mutationFn: d2
  });
  C2 = () => E({
    mutationKey: [
      B
    ],
    mutationFn: u2
  });
  h2 = ({ userSetting: r, updateNotificationSettings: n, updatingNotificationSettings: o, unsubscribing: l, unsubscribe: d }) => {
    const [c, x] = Z.useState([
      ...r
    ]), u = (s, a) => {
      const t = [
        ...c
      ];
      t[s].user = a, x(t);
    }, m = (s) => {
      const a = [
        ...c
      ];
      if (a[s].type === 1) {
        const t = a[s];
        t.user = !t.user;
      } else if (a[s].type === 2) {
        const t = a[s];
        t.enabled = !t.enabled;
      } else {
        const t = a[s];
        t.enabled = !t.enabled;
      }
      x(a);
    }, b = () => {
      n(c);
    }, S = async () => {
      d();
    };
    return e.jsx(i, {
      display: "flex",
      flexDirection: "column",
      alignItems: "flex-start",
      padding: "spacing-xs",
      border: "border-sm solid stroke-secondary",
      backgroundColor: "surface-primary",
      borderRadius: "radius-sm",
      children: e.jsxs(i, {
        display: "flex",
        flexDirection: "column",
        gap: "spacing-xxs",
        alignItems: "center",
        children: [
          e.jsx(i, {
            display: "flex",
            flexDirection: "column",
            width: "-webkit-fill-available",
            maxHeight: "30vh",
            padding: "spacing-none spacing-xs",
            overflow: "scroll",
            customScrollbar: true,
            css: J`
            overflow-x: none;
          `,
            children: c.map((s, a) => e.jsxs(i, {
              children: [
                e.jsxs(i, {
                  display: "flex",
                  flexDirection: "column",
                  padding: "spacing-xs spacing-none",
                  gap: "spacing-xxs",
                  alignSelf: "stretch",
                  alignItems: "flex-start",
                  children: [
                    e.jsxs(i, {
                      display: "flex",
                      flexDirection: "row",
                      justifyContent: "space-between",
                      alignSelf: "stretch",
                      children: [
                        e.jsx(h, {
                          variant: "h6-bold",
                          color: "text-primary",
                          textTransform: "capitalize",
                          children: s.description
                        }),
                        e.jsx(z, {
                          checked: s.type === 1 ? s.user : s.enabled,
                          onCheckedChange: () => m(a)
                        })
                      ]
                    }),
                    s.type === 2 && s.enabled === true && e.jsxs(i, {
                      display: "flex",
                      flexDirection: "column",
                      gap: "spacing-xxs",
                      alignItems: "flex-start",
                      alignSelf: "stretch",
                      children: [
                        e.jsx(h, {
                          variant: "h6-bold",
                          color: "text-primary",
                          textTransform: "capitalize",
                          children: s.user || s.default
                        }),
                        e.jsx(G, {
                          val: s.user,
                          max: s.upperLimit,
                          min: s.lowerLimit,
                          step: s.ticker || 1,
                          defaultVal: s.default,
                          onChange: ({ x: t }) => u(a, t)
                        })
                      ]
                    }),
                    s.type === 3 && s.enabled === true && e.jsxs(i, {
                      display: "flex",
                      flexDirection: "column",
                      gap: "spacing-xxs",
                      alignItems: "flex-start",
                      alignSelf: "stretch",
                      children: [
                        e.jsxs(h, {
                          variant: "h6-bold",
                          color: "text-primary",
                          textTransform: "capitalize",
                          children: [
                            s.user.lower || s.default.lower,
                            " - ",
                            s.user.upper || s.default.upper
                          ]
                        }),
                        e.jsx(Q, {
                          startVal: s.user.lower || s.default.lower,
                          endVal: s.user.upper || s.default.upper,
                          max: s.upperLimit,
                          min: s.lowerLimit,
                          step: s.ticker || 1,
                          defaultStartVal: s.default.lower,
                          defaultEndVal: s.default.upper,
                          onChange: ({ startVal: t, endVal: f }) => u(a, {
                            lower: t,
                            upper: f
                          })
                        })
                      ]
                    })
                  ]
                }),
                e.jsx(W, {})
              ]
            }, a))
          }),
          e.jsxs(i, {
            display: "flex",
            gap: "spacing-xxs",
            alignSelf: "stretch",
            alignItems: "center",
            justifyContent: "flex-end",
            flexDirection: "column",
            padding: "spacing-none spacing-xs",
            children: [
              e.jsx(h, {
                color: "text-tertiary",
                variant: "bes-regular",
                children: "You will receive all important updates from this channel."
              }),
              e.jsxs(i, {
                display: "flex",
                flexDirection: "column",
                gap: "spacing-md",
                alignItems: "center",
                width: "100%",
                children: [
                  e.jsx(Y, {
                    size: "small",
                    variant: "primary",
                    onClick: b,
                    block: true,
                    loading: o,
                    children: o ? "Updating" : "Update Preferences"
                  }),
                  e.jsx(i, {
                    width: "100%",
                    cursor: "pointer",
                    onClick: S,
                    children: e.jsx(h, {
                      textAlign: "center",
                      variant: "bs-semibold",
                      children: l ? "Unsubscribing" : "Unsubscribe"
                    })
                  })
                ]
              })
            ]
          })
        ]
      })
    });
  };
  b2 = ({ children: r, channelDetail: n, onSuccess: o, userSetting: l, profileModalVisibility: d, onChangeProfileModalVisibility: c }) => {
    const { account: x, chainId: u, provider: m, wallet: b } = X(), { handleConnectWalletAndEnableProfile: S } = $(), [s, a] = Z.useState(void 0), { userPushSDKInstance: t } = U((g) => g.user), f = n && (n == null ? void 0 : n.channel_settings) ? JSON.parse(n == null ? void 0 : n.channel_settings) : null, { mutate: H, isPending: A } = C2(), { mutate: R, isPending: K } = p2(), L = e2();
    Z.useEffect(() => {
      (d == null ? void 0 : d.isVisible) && (d == null ? void 0 : d.channel_id) === n.id && t && !(t == null ? void 0 : t.readmode()) && (c == null ? void 0 : c({
        isVisible: false,
        channel_id: null
      }), s ? k(s) : M());
    }, [
      d,
      t
    ]);
    const k = async (g) => {
      if (t && (t == null ? void 0 : t.readmode())) {
        c == null ? void 0 : c({
          isVisible: true,
          channel_id: n.id
        }), a(g);
        return;
      }
      const w = u === v.coreContractChain ? n.channel : n.alias_address, y = t.signer ? t : await S({
        wallet: b
      }) ?? void 0;
      H({
        userPushSDKInstance: y,
        channelAddress: j(w, u),
        settings: s2({
          settings: g
        })
      }, {
        onSuccess: (p) => {
          p.status === 204 ? (o(), L.showMessageToast({
            toastTitle: "Success",
            toastMessage: "Successfully saved the user settings!",
            toastType: "SUCCESS",
            getToastIcon: (C) => e.jsx(V, {
              size: C,
              color: "green"
            })
          })) : L.showMessageToast({
            toastTitle: "Error",
            toastMessage: "There was an error in saving the settings",
            toastType: "ERROR",
            getToastIcon: (C) => e.jsx(I, {
              size: C,
              color: "red"
            })
          });
        },
        onError: (p) => {
          console.log("Error in saving notification settings", p);
        }
      }), a(void 0);
    }, M = async () => {
      if (t && (t == null ? void 0 : t.readmode())) {
        c == null ? void 0 : c({
          isVisible: true,
          channel_id: n.id
        });
        return;
      }
      const T = u === v.coreContractChain ? n.channel : n.alias_address, w = await m.getSigner(x), y = (t == null ? void 0 : t.decryptedPgpPvtKey) ?? t2(x);
      R({
        signer: w,
        channelAddress: j(T, u),
        userAddress: j(x, u),
        env: v.pushNodesEnv,
        decryptedPGPKeys: y
      }, {
        onSuccess: (p) => {
          o(), p.status === 204 ? L.showMessageToast({
            toastTitle: "Success",
            toastMessage: "Successfully opted out of channel !",
            toastType: "SUCCESS",
            getToastIcon: (C) => e.jsx(V, {
              size: C,
              color: "green"
            })
          }) : L.showMessageToast({
            toastTitle: "Error",
            toastMessage: "There was an error opting out of channel",
            toastType: "ERROR",
            getToastIcon: (C) => e.jsx(I, {
              size: C,
              color: "red"
            })
          });
        },
        onError: (p) => {
          console.log("Error in the unsubcribe channel", p);
        }
      });
    };
    return e.jsx(e.Fragment, {
      children: l && f ? e.jsx(P, {
        overlay: e.jsx(h2, {
          userSetting: l ?? n2({
            channelSetting: f
          }),
          updateNotificationSettings: k,
          updatingNotificationSettings: A,
          unsubscribing: K,
          unsubscribe: M
        }),
        children: r
      }) : e.jsx(P, {
        overlay: e.jsx(r2, {
          children: e.jsx(o2, {
            label: "Unsubscribe",
            icon: e.jsx(a2, {}),
            onClick: M
          })
        }),
        children: r
      })
    });
  };
  S2 = () => e.jsxs(i, {
    display: "flex",
    gap: "spacing-xxs",
    padding: "spacing-xxxs",
    children: [
      e.jsxs(h, {
        color: "text-primary-inverse",
        variant: "bs-semibold",
        children: [
          "Verified By:",
          " "
        ]
      }),
      e.jsxs(i, {
        display: "flex",
        gap: "spacing-xxs",
        alignItems: "center",
        children: [
          e.jsx(i2, {
            width: 16,
            height: 16
          }),
          e.jsx(h, {
            color: "text-primary-inverse",
            children: "Push Admin"
          })
        ]
      })
    ]
  });
});
export {
  f2 as N,
  L2 as T,
  b2 as U,
  S2 as V,
  __tla,
  m2 as u
};
