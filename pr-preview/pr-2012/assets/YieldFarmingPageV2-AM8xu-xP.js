...
`;ce.coreContractChain;const Zt=({setActiveTab:t})=>{const{account:s,provider:i}=Ce(),[l,o]=n.useState(null),[r,g]=n.useState(null),[x,d]=n.useState(null),[h,R]=n.useState(null),[v,W]=n.useState(null),[G,D]=n.useState(null),[O,V]=n.useState(null),[Z,$]=n.useState(null),[Y,ie]=n.useState(null),[q,E]=n.useState(null),B=i==null?void 0:i.getSigner(s),F=n.useCallback(async()=>{const M=await Te.getInstance().getPoolStats();D({...M})},[l,r,x,h,v]),m=n.useCallback(async()=>{const M=await Te.getInstance().getPUSHPoolStats();V({...M})},[l,r,x,h,v]),k=n.useCallback(async M=>{const ee=await Te.getInstance().getLPPoolStats(M);$({...ee})},[l,r,x,h,v]),K=n.useCallback(async()=>{const M=await Te.getInstance().getUserData(h);ie({...M})},[h]),Q=n.useCallback(async()=>{const M=await Te.getInstance().getUserData(x);E({...M})},[x]);n.useEffect(()=>{let M=new C(p.pushToken,y.pushToken,B),ee=new C(p.staking,y.staking,B),S=new C(p.depYieldFarmPUSH,y.yieldFarming,B),A=new C(p.depYieldFarmLP,y.yieldFarming,B),z=new C(p.uniswapV2Router02,y.uniswapV2Router02,B);if(o(M),g(ee),R(S),d(A),W(z),B&&s){var w=i.getSigner(s);let P=new C(p.pushToken,y.pushToken,w),u=new C(p.staking,y.staking,w),f=new C(p.depYieldFarmPUSH,y.yieldFarming,w),j=new C(p.depYieldFarmLP,y.yieldFarming,w),U=new C(p.uniswapV2Router02,y.uniswapV2Router02,w);o(P),g(u),R(f),d(j),W(U)}},[s]),n.useEffect(()=>{l!=null&&r!=null&&h!=null&&(Te.getInstance().init(s,l,r,h,x,v),F())},[F]),n.useEffect(()=>{G&&me(G)},[G]);const me=async M=>{m(),k(M),K(),Q()};return e.jsxs(e.Fragment,{children:[e.jsx(wt,{logo:"WarningCircle",title:"This reward program (V1) has ended.",body:"To continue earning rewards please migrate to new pools.",setActiveTab:t}),e.jsxs(Xt,{children:[e.jsx(ht,{poolName:"PUSH",userData:Y,PoolStats:O,poolAddress:p.depYieldFarmPUSH,getUserData:K,getPoolStats:m,tokenAddress:p.pushToken,setActiveTab:t}),e.jsx(ht,{poolName:"UNI-V2",userData:q,PoolStats:Z,poolAddress:p.depYieldFarmLP,getUserData:Q,getPoolStats:k,tokenAddress:p.uniV2LPToken,setActiveTab:t})]})]})},Xt=a(T)`
...
`,Pt=a(ge)`
  ...
  height: 17px;
  width: fit-content;
`,zt=()=>e.jsx(Js,{children:e.jsx(kt,{})})});export{Xs as __tla,zt as default};
