import{bK as p,bR as l,bV as a,__tla as Z}from"./index-CmyWu-jr.js";let W,Y,ee=Promise.all([(()=>{try{return Z}catch{}})()]).then(async()=>{let K,q,G,H,J,_,O,U,Q,X;W=({disabled:e,val:c,min:n,max:t,step:s,defaultVal:u,onChange:y,onDragStart:b,onDragEnd:D,preview:V=!1,...C})=>{const w=l.useRef(null),$=l.useRef(null),g=l.useRef(null),N=l.useRef(null),x=l.useRef(null),M=d=>{e||(b&&b(d),document.addEventListener("mousemove",E),document.addEventListener("mouseup",f),document.addEventListener("touchmove",E),document.addEventListener("touchend",f))},E=d=>{if(e||!N.current)return;const{left:h,width:L}=N.current.getBoundingClientRect(),{clientX:B}=d instanceof MouseEvent?d:d.touches[0];let i=(B-h)/L;const v=u-Math.floor((u-n)/s)*s,P=u+Math.floor((t-u)/s)*s;if(i<=0)i=v;else if(i>=1)i=P;else{const o=Math.floor((i*(t-n)+n-u)/s);i=u+o*s,i<v&&(i=v),i>P&&(i=P)}const F=(s.toString().split(".")[1]||"").length;y({x:Number(i.toFixed(F))})},f=d=>{e||(D&&D(d),document.removeEventListener("mousemove",E),document.removeEventListener("mouseup",f),document.removeEventListener("touchmove",E),document.removeEventListener("touchend",f))},R=()=>{var d;return(d=x.current)==null?void 0:d.style.setProperty("display","flex")},T=()=>{var d;return(d=x.current)==null?void 0:d.style.setProperty("display","none")};return l.useEffect(()=>{var d;if(w.current&&$.current&&g.current){const h=Math.min(t,Math.max(n,c));w.current.style.left=`${(h-n)/(t-n)*98}%`,$.current.style.width=`${(h-n)/(t-n)*100}%`,g.current.style.width=`${(t-h)/(t-n)*100}%`,(d=x.current)==null||d.style.setProperty("left",`${(Number(h)-Number(n))/(Number(t)-Number(n))*90}%`)}},[w,$,g,c,n,t]),a.jsxs(H,{ref:N,onMouseEnter:R,onMouseLeave:T,onTouchStart:R,onTouchEnd:T,...C,children:[a.jsx(q,{ref:$}),a.jsx(K,{ref:w,onTouchStart:M,onMouseDown:M,onTouchEnd:f,onMouseUp:f}),a.jsx(G,{ref:g}),V&&!Number.isNaN(Number(c))&&a.jsx(J,{ref:x,children:c})]})},K=p.div`
  width: 16px;
  height: 16px;
