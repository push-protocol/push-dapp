...
`,K=p.div`
  ...
  align-items: center;
  gap: 10px;
`,Y=({disabled:e,startVal:c,endVal:n,min:t,max:s,step:u,defaultStartVal:y,defaultEndVal:b,onChange:D,onDragStart:V,onDragEnd:C,preview:w=!1,...$})=>{const g=l.useRef(null),N=l.useRef(null),x=l.useRef(null),M=l.useRef(null),E=l.useRef(null),f=l.useRef(null),R=l.useRef(null),T=l.useRef(null),d=o=>{e||(V&&V(o),document.addEventListener("mousemove",h),document.addEventListener("mouseup",L),document.addEventListener("touchmove",h),document.addEventListener("touchend",L))},h=o=>{if(e||!x.current)return;const{left:m,width:z}=x.current.getBoundingClientRect(),{clientX:q}=o instanceof MouseEvent?o:o.touches[0];let r=(q-m)/z;const j=y-Math.floor((y-t)/u)*u,S=y+Math.floor((s-y)/u)*u;if(r<=0)r=j;else if(r>=1)r=S;else{const I=Math.floor((r*(s-t)+t-y)/u);r=y+I*u,r<j&&(r=j),r>S&&(r=S)}const k=(u.toString().split(".")[1]||"").length;Number(r.toFixed(k))>=n||D({startVal:Number(r.toFixed(k)),endVal:n})},L=o=>{e||(C&&C(o),document.removeEventListener("mousemove",h),document.removeEventListener("mouseup",L),document.removeEventListener("touchmove",h),document.removeEventListener("touchend",L))},B=o=>{e||(V&&V(o),document.addEventListener("mousemove",i),document.addEventListener("mouseup",v),document.addEventListener("touchmove",i),document.addEventListener("touchend",v))},i=o=>{if(e||!x.current)return;const{left:m,width:z}=x.current.getBoundingClientRect(),{clientX:q}=o instanceof MouseEvent?o:o.touches[0];let r=(q-m)/z;const j=b-Math.floor((b-t)/u)*u,S=b+Math.floor((s-b)/u)*u;if(r<=0)r=j;else if(r>=1)r=S;else{const I=Math.floor((r*(s-t)+t-b)/u);r=b+I*u,r<j&&(r=j),r>S&&(r=S)}const k=(u.toString().split(".")[1]||"").length;Number(r.toFixed(k))<=c||D({startVal:c,endVal:Number(r.toFixed(k))})},v=o=>{e||(C&&C(o),document.removeEventListener("mousemove",i),document.removeEventListener("mouseup",v),document.removeEventListener("touchmove",i),document.removeEventListener("touchend",v))},P=()=>{var o,m;(o=M.current)==null||o.style.setProperty("display","flex"),(m=E.current)==null||m.style.setProperty("display","flex")},F=()=>{var o,m;(o=M.current)==null||o.style.setProperty("display","none"),(m=E.current)==null||m.style.setProperty("display","none")};return l.useEffect(()=>{var o,m;g.current&&R.current&&N.current&&f.current&&T.current&&(g.current.style.left=`${(c-t)/(s-t)*98}%`,R.current.style.width=`${(c-t)/(s-t)*100}%`,f.current.style.width=`${(n-c)/(s-t)*100}%`,N.current.style.left=`${(n-t)/(s-t)*95}%`,T.current.style.width=`${(s-n)/(s-t)*100}%`,(o=M.current)==null||o.style.setProperty("left",`${(Number(c)-Number(t))/(Number(s)-Number(t))*90}%`),(m=E.current)==null||m.style.setProperty("left",`${(Number(n)-Number(t))/(Number(s)-Number(t))*90}%`))},[g,N,f,R,T,c,n,t,s]),a.jsxs(Q,{ref:x,onMouseEnter:P,onMouseLeave:F,onTouchStart:P,onTouchEnd:F,...$,...$,children:[a.jsx(U,{ref:R}),a.jsx(_,{ref:g,onTouchStart:d,onMouseDown:d,onTouchEnd:L,onMouseUp:L}),a.jsx(O,{ref:f}),a.jsx(_,{ref:N,onTouchStart:B,onMouseDown:B,onTouchEnd:v,onMouseUp:v}),a.jsx(U,{ref:T}),w&&!Number.isNaN(Number(c))&&a.jsx(X,{ref:M,children:c}),w&&!Number.isNaN(Number(n))&&a.jsx(X,{ref:E,children:n})]})},_=p.div`
  width: 16px;
  height: 16px;
  background-color: ${e=>e.theme.default.bg};
  border: 1px solid ${e=>e.theme.default.border};
