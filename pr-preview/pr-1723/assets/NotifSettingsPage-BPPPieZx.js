...
`,Ce=({title:t,description:m,onCancel:f,disabled:y,onClick:i,feeRequired:c})=>{const{account:w,provider:p}=pe(),[a,L]=d.useState(0),[N,C]=d.useState(!1),[B,u]=d.useState(!1),k=ce();d.useEffect(()=>{!w||!p||async function(){const M=await xt({address:w,provider:p,contractAddress:ue.epnscore});L(parseInt(M));const h=parseInt(M);h>=c&&h!=0?C(!0):C(!1)}()},[w,p]);const x=async()=>{if(u(!0),!p)return;const M=p.getSigner(w);k.showLoaderToast({loaderMessage:"Waiting for Confirmation..."});try{const h=await ft({signer:M,contractAddress:ue.epnscore,amount:c-a});console.debug("response",h),h&&(u(!1),L(c),C(!0),k.showMessageToast({toastTitle:"Success",toastMessage:"Successfully approved Push!",toastType:"SUCCESS",getToastIcon:$=>e.jsx(xe,{size:$,color:"green"})}))}catch(h){console.error(h),h.code=="ACTION_REJECTED"?k.showMessageToast({toastTitle:"Error",toastMessage:"User denied message signature.",toastType:"ERROR",getToastIcon:$=>e.jsx(q,{size:$,color:"red"})}):(k.showMessageToast({toastTitle:"Error",toastMessage:"There was an error in approving PUSH Token",toastType:"ERROR",getToastIcon:$=>e.jsx(q,{size:$,color:"red"})}),console.error("Error --> %o",h),console.error({err:h}))}u(!1)};return e.jsxs(e.Fragment,{children:[e.jsxs(Ae,{children:[e.jsxs("div",{children:[e.jsx(De,{children:t}),e.jsx(Le,{children:m})]}),e.jsxs(de,{flex:"0",children:[N?e.jsx(ve,{src:ht}):null,e.jsxs(Me,{children:[c," PUSH"]})]})]}),e.jsx(gt,{noOfPushTokensToCheck:c,containerProps:{width:"100%"},onMintPushToken:async M=>{await mt({noOfTokens:M,provider:p,account:w})}}),B?e.jsx(e.Fragment,{children:e.jsxs(Ie,{children:[e.jsx(Ze,{size:42,color:S.COLORS.PRIMARY_PINK,type:Ye.PROCESSING}),e.jsx(_e,{children:"Verifying Transaction"})]})}):e.jsx(e.Fragment,{children:e.jsxs($e,{children:[e.jsx(ke,{onClick:f,disabled:!1,children:"Cancel"}),a>=c?e.jsx(K,{disabled:y,onClick:i,children:"Save Changes"}):e.jsx(K,{disabled:y,onClick:x,children:"Approve PUSH"})]})})]})},ve=g.img``,Ae=g(le)`
...
`,me=()=>e.jsx(ne,{children:e.jsx(Be,{})}),ne=g(st)`
  ...
  align-self: stretch;
  justify-content: flex-start;
`});export{Dt as __tla,me as default};
