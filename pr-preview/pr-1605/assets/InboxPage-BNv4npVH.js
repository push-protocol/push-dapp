...
`;const rr=je.coreContractChain,ar=({isSpam:e})=>{Sr.pageview(Ie.Inbox);const c=Re(),{account:n,chainId:s,provider:d}=Ne(),{epnsReadProvider:l,epnsCommReadProvider:w}=ie(x=>x.contracts),[f,b]=he.useState(null),y=()=>b(null);fe();const g=rr===s;return he.useEffect(()=>{f&&y()},[f]),he.useEffect(()=>{s&&async function(){const x=g?d:new vr(je.coreRPC),P=new Qe(Er.epnscore,et.epnscore,x),R=jr[s].commAddress,O=new Qe(R,et.epnsComm,d);c(kr(O)),c(Ar(P))}()},[n,s]),he.useEffect(()=>{!l||!w||(l.pushChannelAdmin().then(x=>{c(Tr(x))}).catch(x=>{console.error({err:x})}),l!=null&&w!=null&&(_r.getInstance().init(n,l,w),Cr.getInstance().init(n,l,w,s)))},[l,w]),t.jsx(ir,{children:t.jsxs(nr,{children:[t.jsx("div",{className:"joyride"}),t.jsx(Zt,{isSpam:e}),f&&t.jsx(De,{notification:f,clearToast:y})]})})},ir=u(Rr)`
  align-items: stretch;
  align-self: stretch;
  flex: 1;
  background: ${e=>e.theme.default.bg};
