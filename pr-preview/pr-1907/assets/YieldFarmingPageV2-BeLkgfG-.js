...
`,Ae=i(be)`
  ...
  border-radius: 5px;
  gap: 5px;
`,Rs=({setActiveTab:t})=>{const{provider:s,account:o,chainId:l}=ye(),[r,a]=n.useState(),[h,x]=n.useState(),[c,u]=n.useState(),[v,E]=n.useState(),[_,B]=n.useState(),[N,V]=n.useState(null),[G,O]=n.useState(null),[ee,F]=n.useState(null),[ae,Y]=n.useState(null),[W,k]=n.useState(null),$=s==null?void 0:s.getSigner(o),M=n.useCallback(async()=>{const U=await Re.getInstance().getPoolStats(s);V({...U})},[h,r,v,c,_,s]),w=n.useCallback(async()=>{const U=await Re.getInstance().getPoolStats(s),X=await Re.getInstance().getLPPoolStats(U);O({...X})},[h,r,v,c,_,s]),R=n.useCallback(async()=>{},[]),K=n.useCallback(async()=>{const U=await Re.getInstance().getUserDataLP();F({...U})},[h,r,v,c,_]),we=n.useCallback(async()=>{const[U,X]=await Re.getInstance().getUserDataPUSH(s);k({...U}),Y({...X})},[h,r,v,c,_,s]);return n.useEffect(()=>{if(l!==ge.coreContractChain&&l!==ge.mainnetCoreContractChain)return;O(null),F(null),k(null),Y(null);let U=new b(p.stakingV2,y.stakingV2,$),X=new b(p.pushToken,y.pushToken,$),Te=new b(p.pushCoreV2,y.pushCoreV2,$),j=new b(p.yieldFarmLP,y.yieldFarming,$),m=new b(p.uniswapV2Router02,y.uniswapV2Router02,$);if(x(U),a(X),E(Te),u(j),B(m),$&&o){var g=s==null?void 0:s.getSigner(o);let C=new b(p.stakingV2,y.stakingV2,g),S=new b(p.pushToken,y.pushToken,g),f=new b(p.pushCoreV2,y.pushCoreV2,g),A=new b(p.yieldFarmLP,y.yieldFarming,g),z=new b(p.uniswapV2Router02,y.uniswapV2Router02,g);x(C),a(S),E(f),u(A),B(z)}Re.getInstance().init(o,U,X,Te,j,m),M(),K(),w(),we()},[o,l]),e.jsxs(e.Fragment,{children:[e.jsx(wt,{logo:"announcement",title:"New V2 Pools are now Live! Stake or migrate now.",body:"Users who were part of the previous Push staking program, need to migrate to new pools to continue earning rewards. Click",setActiveTab:t}),e.jsx(ts,{getLpPoolStats:w,poolStats:N,setPoolStats:V}),e.jsx(Qt,{poolStats:N}),e.jsxs(Fs,{children:[e.jsx(Ts,{userDataPush:ae,getUserDataPush:we,PUSHPoolstats:W,getPUSHPoolStats:R}),e.jsx(ls,{lpPoolStats:G,userDataLP:ee,getLpPoolStats:w,getUserDataLP:K})]})]})},Fs=i(T)`
  @media (max-width: 1300px) {
    flex-direction: column;
  }
...
`,Ds=i.div``;ge.coreContractChain;let yt,kt,Pt,rt,Et;yt=()=>{_s.pageview("/yield");const[t,s]=n.useState(0),[o,l]=n.useState(!1),{account:r,chainId:a,switchChain:h}=ye(),x=()=>{const E=ge.allowedNetworks[0];a!==1&&a!==5&&h(E)};n.useEffect(()=>{a!==1&&a!==11155111&&(u(),x())},[a]);const{isModalOpen:c,showModal:u,ModalComponent:v}=Je();return e.jsxs(kt,{children:[e.jsx(v,{InnerComponent:Ms,onConfirm:()=>{},modalPadding:"0px",modalPosition:Ke.ON_PARENT}),e.jsxs(Pt,{children:[e.jsx(rt,{isActive:!t,onClick:()=>{l(!0),s(0)},children:"Yield Farming V2"}),e.jsxs(rt,{isActive:t,onClick:()=>{l(!1),s(1)},children:["Yield Farming V1 ",e.jsx(Et,{children:"Deprecated"})]})]}),t===0&&e.jsx(Rs,{setActiveTab:s}),t===1&&e.jsx(qt,{setActiveTab:s})]})},kt=i(He)`
	align-items: stretch;
	align-self: center;
  flex: 1;
	background: ${t=>t.theme.default.bg};
	border-radius: ${ce.ADJUSTMENTS.RADIUS.LARGE};
	box-shadow: ${ce.ADJUSTMENTS.MODULE_BOX_SHADOW};
	display: flex;
	flex-direction: column;
	flex: initial;
	justify-content: center;
  ...
  width: calc(
    100% - ${Ze.MINI_MODULES.DESKTOP.RIGHT} - ${Ze.MINI_MODULES.DESKTOP.LEFT} -
      ${ce.ADJUSTMENTS.PADDING.BIG} - ${ce.ADJUSTMENTS.PADDING.BIG}
  );
  gap: 16px;
  padding: 24px;

  margin: ${ce.ADJUSTMENTS.MARGIN.MINI_MODULES.DESKTOP};
  
  @media ${qe.laptop} {
    margin: ${ce.ADJUSTMENTS.MARGIN.MINI_MODULES.TABLET};
  }

  @media ${qe.tablet} {
    width: calc(
      100% - ${Ze.MINI_MODULES.TABLET.RIGHT} - ${Ze.MINI_MODULES.TABLET.LEFT} -
        ${ce.ADJUSTMENTS.PADDING.DEFAULT} - ${ce.ADJUSTMENTS.PADDING.DEFAULT}
    );
    margin: ${ce.ADJUSTMENTS.MARGIN.MINI_MODULES.MOBILE};
    padding:16px;
  }

  @media ${qe.mobileL} {
    margin: ${ce.ADJUSTMENTS.MARGIN.MINI_MODULES.MOBILE};
    border: ${ce.ADJUSTMENTS.RADIUS.LARGE};
    width: 100%;
    padding: ${ce.ADJUSTMENTS.PADDING.DEFAULT};
`,Pt=i(T)`
  justify-content: flex-start;
  display: flex;
  flex-direction: row;
