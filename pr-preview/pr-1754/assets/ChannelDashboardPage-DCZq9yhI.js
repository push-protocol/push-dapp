...
`,to=b.p`
  ...
  margin-left: 12px;
  color: ${e=>e.theme.editChannelPrimaryText};
`,io=50,no=Ae.coreContractChain;function ro({DropdownRef:e,isDropdownOpen:n,closeDropdown:i}){ct();const{account:r,chainId:s}=Se(),{epnsWriteProvider:h,epnsCommWriteProvider:a}=Z(M=>M.contracts),{channelDetails:c}=Z(M=>M.admin),{CHANNNEL_DEACTIVATED_STATE:o,CHANNEL_BLOCKED_STATE:d}=Z(M=>M.channels);Z(M=>M.user);const u=pe(),{channelState:l}=c,f=no===s,p=$e(425),{isModalOpen:g,showModal:A,ModalComponent:x}=pt(),{isModalOpen:m,showModal:C,ModalComponent:v}=pt(),{isModalOpen:j,showModal:T,ModalComponent:k}=pt(),E=n&&!g&&!m&&!j;He(e,()=>E&&i()),se.useState(!1),se.useState(io);const[B,R]=se.useState(0),Q=l===o,_=l===d,P=_||Q;se.useEffect(()=>{R(+zc.formatBigNumberToMetric(c.poolContribution,!0))},[r,c.poolContribution]),Te();const U=()=>{_||(Q?C():A())},N=Te(),L=h.reactivateChannel,w=Te(),I=()=>h.deactivateChannel(),S=Te(),D=async(M,F)=>{if(!(M==""||F=="")&&!(M<60))try{const z=M+"+"+F,O=Si(z);return h.addSubGraph(O)}catch(z){console.error(z)}};return t.jsxs(t.Fragment,{children:[t.jsx("div",{children:t.jsx(ao,{background:u,children:t.jsxs(oo,{children:[Ae.appEnv!=="prod"&&t.jsx(an,{disabled:P,onClick:()=>!P&&T(),children:t.jsxs("div",{style:{display:"flex",justifyContent:"flex-start",alignItems:"center"},children:[t.jsx(wt,{src:va,alt:"cube"}),t.jsx("div",{style:{width:"10px"}}),"Add SubGraph Details"]})}),f&&t.jsx(an,{isChannelDeactivated:Q,onClick:U,children:t.jsx(so,{isChannelBlocked:_,isChannelDeactivated:Q,children:t.jsxs("div",{style:{display:"flex",justifyContent:"flex-start",alignItems:"center"},children:[_?t.jsx(wt,{src:en,alt:"red-bell"}):Q?t.jsx(wt,{src:ya,alt:"green-bell"}):t.jsx(wt,{src:en,alt:"red-bell"}),t.jsx("div",{style:{width:"10px"}}),_?"Channel Blocked":Q?"Activate Channel":"Deactivate Channel"]})})})]})})}),t.jsx(x,{InnerComponent:Ba,onConfirm:I,toastObject:w,modalPosition:ut.ON_ROOT}),t.jsx(v,{InnerComponent:Qa,onConfirm:L,toastObject:N,modalMargin:p?"10rem 1rem 0 1rem":"",modalPosition:ut.ON_ROOT}),t.jsx(k,{InnerComponent:Ca,onConfirm:D,toastObject:S,modalPosition:ut.ON_ROOT})]})}const ao=b.div`
  position: absolute;
  right: 20px;
  display: flex;
  flex-direction: column-reverse;
...
`,vc=10;let Pn=null;const yc=()=>{pe();const{account:e,chainId:n,wallet:i}=Se(),{channelDetails:r,delegatees:s,aliasDetails:{aliasAddr:h,aliasEthAddr:a,isAliasVerified:c,aliasAddrFromContract:o}}=Z(_=>_.admin),{processingState:d}=Z(_=>_.channelCreation),{epnsWriteProvider:u}=Z(_=>_.contracts);Z(_=>_.user),y.useContext(ki);const l=r&&r!=="unfetched",f=Te(),p=ct(),[g,A]=y.useState(!1);y.useState(!0);const[x,m]=y.useState(!1),C=Ae.coreContractChain===n,v=$e(600);y.useEffect(()=>{if(!C||!r||o||r==="unfetched")return;const _=cd(r);if(_){const{address:P,chainId:U}=_;p(dd(P)),p(hd(U))}else p(dt(0))},[r,o]);const j=async _=>{let{aliasAddress:P=null,isAliasVerified:U=null}=await Jn.getInstance().getChannelDetailsFromAddress(_);return P=="NULL"&&(P=null),{aliasAddress:P,aliasVerified:U}};y.useEffect(()=>{!C||!o||d===0||(Pn=setInterval(async()=>{const{aliasAddress:_,aliasVerified:P}=await j(e);_?(p(pd(_)),P?(p(Di(!0)),p(dt(0))):(p(dt(2)),p(Di(!1)))):d!=0&&d!=1&&p(dt(1))},vc*1e3))},[o]),h!==null&&clearInterval(Pn);const T=async()=>{try{f.showLoaderToast({loaderMessage:"Waiting for Confirmation..."});const _=await u.destroyTimeBoundChannel(e,{gasLimit:1e6});console.debug(_),console.debug("Check: "+e),await _.wait(),f.showMessageToast({toastTitle:"Success",toastMessage:"Successfully deleted the channel",toastType:"SUCCESS",getToastIcon:P=>t.jsx(ie,{size:P,color:"green"})}),p(Bi(null))}catch(_){console.error(_),_.code=="ACTION_REJECTED"?f.showMessageToast({toastTitle:"Error",toastMessage:"User denied message signature.",toastType:"ERROR",getToastIcon:P=>t.jsx(ie,{size:P,color:"red"})}):f.showMessageToast({toastTitle:"Error",toastMessage:"There was an error in deleting the channel",toastType:"ERROR",getToastIcon:P=>t.jsx(ie,{size:P,color:"red"})})}},k=()=>{m(!0)},E=()=>{m(!1)},{isModalOpen:B,showModal:R,ModalComponent:Q}=pt();return t.jsxs(te,{children:[(r==="unfetched"||d===null)&&t.jsx(Xi,{}),r!=="unfetched"&&t.jsxs(ue,{justifyContent:d===0&&"flex-start",height:"fit-content",children:[!r&&d===0&&t.jsx(Pl,{}),l&&d!==null&&t.jsx(t.Fragment,{children:x?t.jsx(sc,{closeEditChannel:E,UploadLogoComponent:Q,displayUplaodLogoModal:R,isUploadLogoModalOpen:B}):t.jsxs(t.Fragment,{children:[r&&!v&&t.jsxs(te,{position:"absolute",top:"0",right:"0",zIndex:"1",gap:"8px",children:[!g&&C&&t.jsx(ge,{onClick:k,size:"small",variant:"outline",children:"Edit Channel"}),!g&&t.jsx(on,{}),g&&C&&t.jsx(ge,{onClick:T,size:"small",variant:"danger",children:"Delete Channel"})]}),r?t.jsx(ho,{isChannelExpired:g,setIsChannelExpired:A,showEditChannel:k,destroyChannel:T}):""]})}),d!==0&&d!==null&&l&&!x&&t.jsx(t.Fragment,{children:t.jsx(da,{aliasEthAccount:a,setAliasVerified:Di})})]})]})};Ae.coreContractChain;function Cc(){ud.pageview("/channel_dashboard");const[e,n]=se.useState(!0),[i,r]=se.useState(null),s=()=>r(null);return se.useEffect(()=>{i&&s()},[i]),t.jsxs(Ic,{children:[e?t.jsx(yc,{}):t.jsx(Xi,{}),i&&t.jsx(md,{notification:i,clearToast:s})]})}const Ic=b(Me)`
  align-items: center;
  align-self: center;
