...
`,de=n(Ie)`
  cursor: pointer;
`;function he(){var s,h;const{account:t,chainId:o}=O(),{userPushSDKInstance:r}=A(a=>a.user),{subscriptionStatus:f,userSettings:x}=A(a=>a.channels),[g,w]=d.useState(0),[_,y]=d.useState([]),[T,j]=d.useState(!0);H();const b=z(),L=async a=>{const l=await r.channel.info(a);if(l)return{channel:a,id:l.id,icon:l.icon,name:l.name,channel_settings:l.channel_settings}},E=async a=>{const l=[];await Promise.all(Object.keys(a).map(async S=>{const M=await L(S);M&&l.push(M)})),y(l)};d.useEffect(()=>{!t||!r||async function(){if(j(!0),Object.keys(f).length===0){const a=await r.notification.subscriptions(),l={},S={};a.map(({channel:M,user_settings:C})=>{l[M]=!0,S[M]=C?JSON.parse(C):null}),b(R(l)),b(G(S)),await E(l)}else await E(f);j(!1)}()},[t,r]);const i=[{value:0,label:"Notification Settings",title:"Notification Settings"},{value:1,label:"Push Snap",title:""}];return e.jsxs(pe,{children:[e.jsx(xe,{children:"Settings"}),e.jsx(ge,{children:"Customize your Push profile or manage your notification preferences"}),e.jsxs(ue,{children:[e.jsx(fe,{children:i.map(a=>e.jsx(Le,{onClick:()=>w(a.value),isSelected:a.value===g,children:a.label},a.value))}),e.jsx(me,{children:e.jsxs(Se,{children:[((s=i[g])==null?void 0:s.title)&&e.jsx(we,{children:(h=i[g])==null?void 0:h.title}),g===0&&e.jsx(te,{}),g===1&&e.jsx(oe,{})]})})]})]})}const pe=n.div`
  padding: 32px 24px;
  flex: 1;
...
`,n.div`
  ...
  align-items: center;
  height: 80%;
`;let N,Z,F;N=()=>e.jsx(Z,{children:e.jsx(he,{})}),Z=n.div`
	align-items: stretch;
	align-self: stretch;
  flex: 1;
	background: ${t=>t.theme.default.bg};
